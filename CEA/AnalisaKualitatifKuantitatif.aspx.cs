...
{
	/// <summary>
	/// Summary description for AnalisaKualitatifKuantitatif.
	/// </summary>
	public partial class AnalisaKualitatifKuantitatif : System.Web.UI.Page
	{
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			try {TXT_TOTAL_QUAL.Text= DGR_QUALR.Items[0].Cells[0].Text.Trim() ;
		...
		private void BindDataQualR()
		{
				...
				if (LBL_RFREKANANTYPE.Text=="01")//Akreditasi Perusahaan Penilai
				{
					conn.QueryString="Select avg(score)*0.25 as SUM from REKANAN_QUALITATIVE where regnum='" + Request.QueryString["regnum"] + "' ";
					conn.ExecuteQuery();
				}
				else if (LBL_RFREKANANTYPE.Text=="02")//Akreditasi Kantor Akuntan Publik
				{
					conn.QueryString="Select avg(score)*0.3 as SUM from REKANAN_QUALITATIVE where regnum='" + Request.QueryString["regnum"] + "' ";
					conn.ExecuteQuery();
				}
				else if (LBL_RFREKANANTYPE.Text=="03")//Konsultan Manajemen
				{
					conn.QueryString="Select avg(score)*0.4 as SUM from REKANAN_QUALITATIVE where regnum='" + Request.QueryString["regnum"] + "' ";
					conn.ExecuteQuery();
				}
				else if (LBL_RFREKANANTYPE.Text=="04")//Perusahaan Asuransi
				{
