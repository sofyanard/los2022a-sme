...
{
	/// <summary>
	/// Summary description for ListRekanan.
	/// </summary>
	public partial class ListRekanan : System.Web.UI.Page
	{
		...
		private void DatGrd_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "View":	
					...
					{
						conn.QueryString = "select regnum from APPLICATION_REKANAN where REKANAN_REF = '" + e.Item.Cells[0].Text + "' and REGNUM = '" + e.Item.Cells[0].Text + "C" + "' and CBI = '1'";
						conn.ExecuteQuery();

						if(conn.GetRowCount() == 0)
						{
							conn.QueryString = "insert into APPLICATION_REKANAN (regnum, rekanan_ref, CBI) " + 
								"values ('" + e.Item.Cells[0].Text + "C" + "', '" + e.Item.Cells[0].Text + "', '1')";
							conn.ExecuteQuery();
						...
						//conn.ExecuteQuery();
						Response.Redirect("RekananInfo.aspx?sta=exist&rekanan_ref=" + e.Item.Cells[0].Text + "&tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"] + "&regnum=" + conn.GetFieldValue(0,0) + "&exist=1");
		...
		{
			string rekanan_ref, regnum;
			conn.QueryString = "exec REKANAN_GENERATE_ID '" + Session["BranchID"].ToString() + "', '1'";
			conn.ExecuteQuery();

			if (conn.GetRowCount() > 0)
			{
				rekanan_ref = conn.GetFieldValue(0,0);
			}
			else
			{
				return;
			...
			conn.QueryString = "select regnum from APPLICATION_REKANAN where rekanan_ref = '" + rekanan_ref + "' and regnum = '" + rekanan_ref + "C" + "' and CBI = '1'";
			conn.ExecuteQuery();

			if(conn.GetRowCount() == 0)
			{
				conn.QueryString = "insert into APPLICATION_REKANAN (regnum, rekanan_ref, CBI) " + 
					"values ('" + rekanan_ref + "C" + "', '" + rekanan_ref + "', '1')";
			...
			Response.Redirect("RekananInfo.aspx?sta=baru&rekanan_ref=" + rekanan_ref + "&regnum=" + regnum + "&tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"]);
			//Response.Redirect("RekananInfo.aspx?regnum=" + conn.GetFieldValue(0,0) + "&rekanan_ref=" + conn.GetFieldValue(0,1) + "&tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"] + "&exist=0");
		}
