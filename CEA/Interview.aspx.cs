...
namespace SME.CEA
{
	...
	public partial class Interview : System.Web.UI.Page
	{
		...
		protected string[] nons = new string[3];
		...
		private void cekFIELD(string kodeJenisData, string rekref, string regnum, string user, string stat)
		{
			...
			string tglWW = tool.ConvertDate(LBL_DAY.Text, LBL_IVW_MONTH.Text, LBL_YEAR.Text);
			string tglWWNew = tool.ConvertDate(TXT_DAY.Text, DDL_IVW_MONTH.SelectedValue, TXT_YEAR.Text);
			if(tglWW!=tglWWNew)
			{	
				temp="Tgl Wawancara: ";
				try
			...
			if(LBL_PESERTA1.Text!=TXT_PESERTA1.Text)
			{
				temp="Nama Peserta: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_PESERTA1.Text + "', '" +
						temp + TXT_PESERTA1.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
				{
			...
			if(LBL_PESERTA2.Text!=TXT_PESERTA2.Text)
			{
				temp="Nama Peserta: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_PESERTA2.Text + "', '" +
						temp + TXT_PESERTA2.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
				{
			...
			if(LBL_PESERTA3.Text!=TXT_PESERTA3.Text)
			{
				temp="Nama Peserta: ";
				try
				{
			...
			{	
				...
				Label lbl = (Label) DGR_IVW.Items[i].Cells[8].FindControl("LBL_RADIOBUTTON_HIST");
			
				if (lbl.Text != rblinterview.SelectedValue)
				{
					temp="Skala " + DGR_IVW.Items[i].Cells[2].Text + " : ";
					try
					{						
						...
						conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
							sqlpar + ", '" +
							temp + lbl.Text+ "', '" +
							temp + rblinterview.SelectedValue + "'"; 
						conn.ExecuteNonQuery(); 
					}				
			...
			if(LBL_SUB_TOT1.Text!=SUB_TOT1.Text)
			{
				temp="Nilai total Pengalaman Kerja/Keahlian Khusus Calon Rekanan: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_SUB_TOT1.Text + "', '" +
						temp + SUB_TOT1.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_COMENT1.Text!=COMENT1.Text)
			{
				temp="Komentar Pengalaman Kerja/Keahlian Khusus Calon Rekanan: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_COMENT1.Text + "', '" +
						temp + COMENT1.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_SUB_TOT2.Text!=SUB_TOT2.Text)
			{
				temp="Nilai total Kondisi tenaga Kerja/ahli Calon Rekanan: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_SUB_TOT2.Text + "', '" +
						temp + SUB_TOT2.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_COMENT2.Text!=COMENT2.Text)
			{
				temp="Komentar Kondisi tenaga Kerja/ahli Calon Rekanan Calon Rekanan: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_COMENT2.Text + "', '" +
						temp + COMENT2.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_SUB_TOT3.Text!=SUB_TOT3.Text)
			{
				temp="Nilai total Pengendalian mutu yang dimiliki Calon Rekanan: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_SUB_TOT3.Text + "', '" +
						temp + SUB_TOT3.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_COMENT3.Text!=COMENT3.Text)
			{
				temp="Komentar Pengendalian mutu yang dimiliki Calon Rekanan: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_COMENT3.Text + "', '" +
						temp + COMENT3.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_SUB_TOT4.Text!=SUB_TOT4.Text)
			{
				temp="Nilai total Daya saing biaya yang dikenakan dalam mengerjakan suatu proyek /pekerjaan dibandingkan dgn perusahaan lain: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_SUB_TOT4.Text + "', '" +
						temp + SUB_TOT4.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_COMENT4.Text!=COMENT4.Text)
			{
				temp="Komentar Daya saing biaya yang dikenakan dalam mengerjakan suatu proyek /pekerjaan dibandingkan dgn perusahaan lain: ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_COMENT4.Text + "', '" +
						temp + COMENT4.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_SUB_TOT5.Text!=SUB_TOT5.Text)
			{
				temp="Nilai total Lain-lain (Kerjasama dgn instansi lain, organisasi asing, jaringan kerja): ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_SUB_TOT5.Text + "', '" +
						temp + SUB_TOT5.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
			...
			if(LBL_COMENT5.Text!=COMENT5.Text)
			{
				temp="Komentar Lain-lain (Kerjasama dgn instansi lain, organisasi asing, jaringan kerja): ";
				try
				{
					conn.QueryString = "EXEC REKANAN_AUDITTRAIL_INSERT '"+
						sqlpar + ", '" +
						temp + LBL_COMENT5.Text + "', '" +
						temp + COMENT5.Text + "'"; 
					conn.ExecuteNonQuery(); 
				}
				catch (Exception ex)
		...
		private void ViewData()
		{
			...
			LBL_DAY.Text=tool.FormatDate_Day(conn.GetFieldValue("INTVIEW_DATE"));

			try{
				DDL_IVW_MONTH.SelectedValue = tool.FormatDate_Month(conn.GetFieldValue("INTVIEW_DATE"));
				LBL_IVW_MONTH.Text = tool.FormatDate_Month(conn.GetFieldValue("INTVIEW_DATE"));
			}
			catch{DDL_IVW_MONTH.SelectedValue="";}

			TXT_YEAR.Text=tool.FormatDate_Year(conn.GetFieldValue("INTVIEW_DATE"));
			LBL_YEAR.Text=tool.FormatDate_Year(conn.GetFieldValue("INTVIEW_DATE"));

			TXT_DILAKSANAKAN1.Text=conn.GetFieldValue("INTVIEWER1");
			LBL_DILAKSANAKAN1.Text=conn.GetFieldValue("INTVIEWER1");

			TXT_DILAKSANAKAN2.Text=conn.GetFieldValue("INTVIEWER2");
			LBL_DILAKSANAKAN2.Text=conn.GetFieldValue("INTVIEWER2");

			TXT_PESERTA1.Text=conn.GetFieldValue("CANDIDATE1");
			LBL_PESERTA1.Text=conn.GetFieldValue("CANDIDATE1");

			TXT_PESERTA2.Text=conn.GetFieldValue("CANDIDATE2");
			LBL_PESERTA2.Text=conn.GetFieldValue("CANDIDATE2");

			TXT_PESERTA3.Text=conn.GetFieldValue("CANDIDATE3");
			LBL_PESERTA3.Text=conn.GetFieldValue("CANDIDATE3");

			TXT_SUM.Text=conn.GetFieldValue("NONS_TOT");

			SUB_TOT1.Text=conn.GetFieldValue("S_EXPERIANCE");
			LBL_SUB_TOT1.Text=conn.GetFieldValue("S_EXPERIANCE");

			COMENT1.Text=conn.GetFieldValue("S_EXP_COMMENT");
			LBL_COMENT1.Text=conn.GetFieldValue("S_EXP_COMMENT");

			SUB_TOT2.Text=conn.GetFieldValue("S_EXPERT");
			LBL_SUB_TOT2.Text=conn.GetFieldValue("S_EXPERT");

			COMENT2.Text=conn.GetFieldValue("S_EXPERT_COMMENT");
			LBL_COMENT2.Text=conn.GetFieldValue("S_EXPERT_COMMENT");

			SUB_TOT3.Text=conn.GetFieldValue("S_MUTU");
			LBL_SUB_TOT3.Text=conn.GetFieldValue("S_MUTU");

			COMENT3.Text=conn.GetFieldValue("S_MUTU_COMMENT");
			LBL_COMENT3.Text=conn.GetFieldValue("S_MUTU_COMMENT");

			SUB_TOT4.Text=conn.GetFieldValue("S_COST");
			LBL_SUB_TOT4.Text=conn.GetFieldValue("S_COST");

			COMENT4.Text=conn.GetFieldValue("S_COST_COMMENT");
			LBL_COMENT4.Text=conn.GetFieldValue("S_COST_COMMENT");

			SUB_TOT5.Text=conn.GetFieldValue("S_OTHERS");
			LBL_SUB_TOT5.Text=conn.GetFieldValue("S_OTHERS");

			COMENT5.Text=conn.GetFieldValue("S_OTHERS_COMMENT");
			LBL_COMENT5.Text=conn.GetFieldValue("S_OTHERS_COMMENT");

			SUB_TOT.Text=conn.GetFieldValue("S_TOT");
			TOT.Text=conn.GetFieldValue("SC_TOTAL");
			CAT.Text=conn.GetFieldValue("cat");
			LBL_CAT.Text=conn.GetFieldValue("cat");			
			...
			if(conn.GetRowCount()>0)
			{
				...
				string delivary = conn.GetFieldValue(0,2);
					...
					{		
						...
						Label lbl = (Label) DGR_IVW.Items[i].Cells[8].FindControl("LBL_RADIOBUTTON_HIST");
						try{
							rblinterview.SelectedValue = conn.GetFieldValue(0,i);
							lbl.Text=rblinterview.SelectedValue;
						}
						catch{}
		...
		private void RadioButton86_CheckedChanged(object sender, System.EventArgs e)
		{
