...
{
	/// <summary>
	/// Summary description for DokumenLegalitas.
	/// </summary>
	public partial class DokumenLegalitas : System.Web.UI.Page
	{
		...
		private void FillDDLDoc()
		{
			...
			conn.QueryString = "SELECT * FROM VW_SDC_DDLJENISDOCUMENT_LEGALITAS ORDER BY CERTTYPEDESC";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
		...
		{
			conn.QueryString = "SELECT * FROM VW_SDC_DOC_UMUM WHERE CU_REF = '" + Request.QueryString["curef"] + "' AND DOC_GROUP = '1'";
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				conn.QueryString = "EXEC SDC_DOC_UMUM_INSERT '1','" +
									LBL_SEQ.Text + "','" +
		...
		private void DATA_GRID_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				case "edit":
					conn.QueryString = "SELECT * FROM SDC_DOC_UMUM WHERE SEQ = '" + e.Item.Cells[0].Text.ToString() + "' AND CU_REF = '" + Request.QueryString["curef"] + "' AND DOC_GROUP = '1'";
					conn.ExecuteQuery();
				...
				case "delete":
					conn.QueryString = "DELETE SDC_DOC_UMUM WHERE SEQ = '" + e.Item.Cells[0].Text.ToString() + "' AND CU_REF = '" + Request.QueryString["curef"] + "' AND DOC_GROUP = '1'";
					conn.ExecuteQuery();
