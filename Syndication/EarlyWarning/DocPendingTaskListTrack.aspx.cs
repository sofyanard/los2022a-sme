...
{
	/// <summary>
	/// Summary description for DocPendingTaskListTrack.
	/// </summary>
	public partial class DocPendingTaskListTrack : System.Web.UI.Page
	{
		...
		private void FillDDLType()
		{
			...
			conn.QueryString = "SELECT DISTINCT TEMPLATE_ID, TEMPLATE_DESC FROM DOCEXPORT_TEMPLATE_MASTER WHERE TEMPLATE_GROUP = 'SDCPENDINGTASK'";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
		...
		{
			conn.QueryString = "SELECT * FROM VW_DOCEXPORT_VIEWFILEEXPORT WHERE TEMPLATE_GROUP = 'SDCPENDINGTASK' AND FE_USERID = '" + Session["UserID"].ToString() + "'";
			conn.ExecuteQuery();
				...
				try
				{
					...
					if (dt1.Rows.Count > 0)
					{
						for (int i = 0; i < dt1.Rows.Count; i++)
						{
							...
							/* Procedure :
							 * SDC_ACTION_TICKLER_REPORTING
							 * SDC_TGL_DOK_JAMINAN_REPORTING
							 * SDC_TGL_DOK_OTHER_REPORTING
							 * */
		...
		private void DATA_EXPORT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					try 
					{					
						conn.QueryString = "SELECT TOP 1 UPLOAD_PATH FROM VW_DOCEXPORT_PARAMETER WHERE TEMPLATE_GROUP = 'SDCPENDINGTASK'";
						conn.ExecuteQuery();
