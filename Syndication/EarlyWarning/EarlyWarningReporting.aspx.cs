...
{
	/// <summary>
	/// Summary description for EarlyWarningReporting.
	/// </summary>
	public partial class EarlyWarningReporting : System.Web.UI.Page
	{
		...
		private void setTableItem()
		{
			...
			try
			{
				conn.QueryString = "SELECT * FROM VW_SDC_CHOOSE_DASHBOARD_TYPE";
				conn.ExecuteQuery();
			}
			catch (Exception a)
			{
			...
			foreach (Control ctrl in Page.Controls)
			{
				if(ctrl is HtmlInputRadioButton)
				{
					...
					if(ctrlfix.Checked)
					{
						try
						{
							conn.QueryString = "SELECT * FROM VW_SDC_CHOOSE_DASHBOARD_TYPE WHERE ID = '" + ctrlfix.Value.ToString() + "'";
							conn.ExecuteQuery();
