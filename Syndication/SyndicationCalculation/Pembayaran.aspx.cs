...
{
	/// <summary>
	/// Summary description for Pembayaran.
	/// </summary>
	public partial class Pembayaran : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				FillDDLProduct();
				FillDDLType();
				FillDDLKolektibilitas();
				FillViewData();

				DDL_PEMBAYARAN_MONTH.Items.Add(new ListItem("--Select--",""));
				for (int i = 1; i <= 12; i++)
				{
					DDL_PEMBAYARAN_MONTH.Items.Add(new ListItem(DateAndTime.MonthName(i, false), i.ToString()));
				}
			}
			FillDataGrid();
		}
		...
		{
			/*conn.QueryString = "EXEC SDC_TOTAL_KEWAJIBAN_INFO '" + Request.QueryString["curef"] + "','" + DDL_BANK_TYPE.SelectedValue + "','" + DDL_PRODUCT_TYPE.SelectedValue + "'";
			conn.ExecuteQuery();

			TXT_KEWAJIBAN_POKOK.Text		= tools.MoneyFormat(conn.GetFieldValue("KEW_POKOK").ToString());
			TXT_KEWAJIBAN_BUNGA.Text		= tools.MoneyFormat(conn.GetFieldValue("KEW_BUNGA").ToString());*/
		}

		private void FillDDLType()
		{
			DDL_TYPE_PEMBAYARAN.Items.Clear();
			DDL_TYPE_PEMBAYARAN.Items.Add(new ListItem("--Select--", ""));

			conn.QueryString = "SELECT CODE, [DESC] FROM VW_SDC_TYPE_PENARIKAN_PEMBAYARAN WHERE CODE IN ('4','5','6')";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
				DDL_TYPE_PEMBAYARAN.Items.Add(new ListItem(conn.GetFieldValue(i,1), conn.GetFieldValue(i,0)));
			}
		}

		private void FillDDLKolektibilitas()
		{
			DDL_KOLEKTIBILITAS.Items.Clear();
			DDL_KOLEKTIBILITAS.Items.Add(new ListItem("--Select--", ""));

			conn.QueryString = "SELECT * FROM VW_SDC_RFCOLLECTABILITY";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
		...
		{
			conn.QueryString = "SELECT * FROM VW_SDC_CALCULATION_PEMBAYARAN WHERE CU_REF = '" + Request.QueryString["curef"] + "' ORDER BY BANK_NM, PRODUCT_NM, TRX_CODE";
		...
		{
			if (TXT_PEMBAYARAN_DAY.Text != "" && DDL_PEMBAYARAN_MONTH.SelectedValue != "" && TXT_PEMBAYARAN_YEAR.Text != "") 
			{
				if (!GlobalTools.isDateValid(TXT_PEMBAYARAN_DAY.Text, DDL_PEMBAYARAN_MONTH.SelectedValue, TXT_PEMBAYARAN_YEAR.Text)) 
				{
					GlobalTools.popMessage(this, "Tanggal Pembayaran Tidak Valid!");
					return;
				}
			}
			...
			try
			{
				conn.QueryString = "EXEC SDC_CALCULATION_PEMBAYARAN_INSERT '" +
									LBL_SEQ.Text + "','" +
									...
									TXT_KEWAJIBAN_POKOK.Text.Replace(".","").Replace(",",".") + "','" +
									TXT_KEWAJIBAN_BUNGA.Text.Replace(".","").Replace(",",".") + "','" +
									DDL_KOLEKTIBILITAS.SelectedValue + "','" +
									DDL_TYPE_PEMBAYARAN.SelectedValue + "','" +
									TXT_PEMBAYARAN.Text.Replace(".","").Replace(",",".") + "','" +
									/*TXT_PEMBAYARAN_POKOK.Text.Replace(",",".") + "','" +
									TXT_PEMBAYARAN_BUNGA.Text.Replace(",",".") + "','" +
									TXT_PEMBAYARAN_NCL.Text.Replace(",",".") + "','" +*/
									TXT_COMMITMENT_FEE.Text.Replace(".","").Replace(",",".") + "'," +
									tools.ConvertDate(TXT_PEMBAYARAN_DAY.Text, DDL_PEMBAYARAN_MONTH.SelectedValue, TXT_PEMBAYARAN_YEAR.Text) + ",'" +
									TXT_REMARK.Text + "'";
				conn.ExecuteQuery();
		...
		private void ClearData()
		{
			...
			DDL_BANK_TYPE.SelectedValue			= "";
			DDL_PRODUCT_TYPE.SelectedValue		= "";
			TXT_KEWAJIBAN_POKOK.Text			= "";
			TXT_KEWAJIBAN_BUNGA.Text			= "";
			DDL_KOLEKTIBILITAS.SelectedValue	= "";
			DDL_TYPE_PEMBAYARAN.SelectedValue	= "";
			TXT_PEMBAYARAN.Text					= "";
			/*TXT_PEMBAYARAN_POKOK.Text			= "";
			TXT_PEMBAYARAN_BUNGA.Text			= "";
			TXT_PEMBAYARAN_NCL.Text				= "";*/
			TXT_COMMITMENT_FEE.Text				= "";
			TXT_PEMBAYARAN_DAY.Text				= "";
			DDL_PEMBAYARAN_MONTH.SelectedValue	= "";
			TXT_PEMBAYARAN_YEAR.Text			= "";
			TXT_REMARK.Text						= "";
		}
		...
		private void DATA_GRID_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "edit":
					...
					TXT_KEWAJIBAN_POKOK.Text			= tools.MoneyFormat(conn.GetFieldValue("KEW_POKOK").ToString().Replace("&nbsp;",""));
					TXT_KEWAJIBAN_BUNGA.Text			= tools.MoneyFormat(conn.GetFieldValue("KEW_BUNGA").ToString().Replace("&nbsp;",""));
					DDL_KOLEKTIBILITAS.SelectedValue	= conn.GetFieldValue("KOLE_CODE").ToString();
					DDL_TYPE_PEMBAYARAN.SelectedValue	= conn.GetFieldValue("TRX_CODE").ToString();
					TXT_PEMBAYARAN.Text					= tools.MoneyFormat(conn.GetFieldValue("PEMBAYARAN").ToString().Replace("&nbsp;",""));
					/*TXT_PEMBAYARAN_POKOK.Text			= conn.GetFieldValue("PEMBAYARAN_POKOK").ToString().Replace("&nbsp;","");
					TXT_PEMBAYARAN_BUNGA.Text			= conn.GetFieldValue("PEMBAYARAN_BUNGA").ToString().Replace("&nbsp;","");
					TXT_PEMBAYARAN_NCL.Text				= conn.GetFieldValue("PEMBAYARAN_NCL").ToString().Replace("&nbsp;","");*/
					TXT_COMMITMENT_FEE.Text				= tools.MoneyFormat(conn.GetFieldValue("COMMITMENT_FEE").ToString().Replace("&nbsp;",""));
					TXT_PEMBAYARAN_DAY.Text				= tools.FormatDate_Day(conn.GetFieldValue("TRX_DATE").ToString());
					DDL_PEMBAYARAN_MONTH.SelectedValue	= tools.FormatDate_Month(conn.GetFieldValue("TRX_DATE").ToString());
					TXT_PEMBAYARAN_YEAR.Text			= tools.FormatDate_Year(conn.GetFieldValue("TRX_DATE").ToString());
		...
		private void DGR_VIEWDATA_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "view":
					...
					conn.QueryString = "EXEC SDC_TOTAL_KEWAJIBAN_INFO '" + Request.QueryString["curef"] + "','" + DDL_BANK_TYPE.SelectedValue + "','" + DDL_PRODUCT_TYPE.SelectedValue + "'," + LBL_PRODUCT_SEQ.Text;
					conn.ExecuteQuery();
