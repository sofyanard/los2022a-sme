namespace SME.LMS.PortfolioWatchlistChecking.CommonForm
{
	using System;
	using System.IO;
	...
	public partial class DocExport : System.Web.UI.UserControl
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ((Request.QueryString["porlmsregno"] != null) && (Request.QueryString["porlmsregno"] != ""))
				regnum = Request.QueryString["porlmsregno"];
			...
			if (!IsPostBack)
			{
				string qryddl = "SELECT DISTINCT TEMPLATE_ID, TEMPLATE_DESC FROM PORTFOLIO_DOCEXPORT_TEMPLATE WHERE TEMPLATE_GROUP = '" + vargrouptemplate + "'";;
		...
		private string CreateWord(string templateid)
		{
			...
			conn.QueryString = "SELECT TOP 1 * FROM VW_LMSPOR_DOCEXPORT_PARAMETER WHERE TEMPLATE_ID = '" + templateid + "'";
			conn.ExecuteQuery();

			if (conn.GetRowCount() > 0)
			{
				templatefilename = conn.GetFieldValue("TEMPLATE_FILENAME");
				outputfilename = conn.GetFieldValue("UPLOAD_FILEFORMAT").Replace("#REGNUM$",regnum).Replace("#USERID$",Session["UserID"].ToString()) + "-" + curr_date + ".DOC";
				...
				try
				{
					...
					{
						string dfdfg = o.Message.ToString();
						string sdfsg = o.Message.ToString();
					}
					Word.Bookmarks wordBookMark = (Word.Bookmarks)wordDoc.Bookmarks;

					conn.QueryString = "SELECT SHEET_ID, SHEET_SEQ, STOREDPROCEDURE FROM VW_LMSPOR_DOCEXPORT_PARAMETER WHERE TEMPLATE_ID = '" + templateid + "'";
					conn.ExecuteQuery();
					...
					if (dt1.Rows.Count > 0)
					{
						for (int i = 0; i < dt1.Rows.Count; i++)
						{
							...
							conn.QueryString = "EXEC " + proc + " '" + regnum + "'";
							conn.ExecuteQuery();
							dt3 = conn.GetDataTable().Copy();

							//Loop for Template Detail
							conn.QueryString = "SELECT CELL_ROW, CELL_COL, DB_FIELD FROM PORTFOLIO_DOCEXPORT_TEMPLATE_DETAIL WHERE SHEET_ID = '" + 
						...
						{
							//Save to Table
							conn.QueryString = "EXEC LMSPOR_DOCEXPORT_SAVE '1', '" + 
								templateid + "', '" + 
								regnum + "', '" + 
		...
		private void ViewExportFiles()
		{
			...
			conn.QueryString = "SELECT * FROM VW_LMSPOR_DOCEXPORT_VIEWFILEEXPORT WHERE TEMPLATE_GROUP = '" + vargrouptemplate + 
				"' AND REGNUM = '" + regnum + "'";
			conn.ExecuteQuery();
		...
		protected void BTN_EXPORT_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				conn.QueryString = "SELECT DOC_TYPE, ACTION_TYPE FROM PORTFOLIO_DOCEXPORT_TEMPLATE WHERE TEMPLATE_ID = '" + DDL_FORMAT_TYPE.SelectedValue + "'";
				conn.ExecuteQuery();
		...
		private void DATA_EXPORT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					try 
					{					
						conn.QueryString = "SELECT TOP 1 UPLOAD_PATH FROM VW_LMSPOR_DOCEXPORT_PARAMETER WHERE TEMPLATE_GROUP = '" + vargrouptemplate + "'";
						conn.ExecuteQuery();
						...
						if (conn.GetRowCount() > 0)
						{
							...
							conn.QueryString = "EXEC LMSPOR_DOCEXPORT_SAVE '2', '" + e.Item.Cells[1].Text +
