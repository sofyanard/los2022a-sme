...
{
	/// <summary>
	/// Summary description for AdvisPortRevWC.
	/// </summary>
	public partial class AdvisPortRevWC : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				conn.QueryString = "select * from VW_PORTFOLIO_WC_USER_HISTORY where trackcode='P3.1' and por_tracknextby='" + Session["UserID"].ToString() +"'";	
				conn.ExecuteQuery();
		...
		private void SearchData()
		{
			...
			if(query!="")
			{
				conn.QueryString="select * from VW_PORTFOLIO_WC_USER_HISTORY where trackcode='P3.1' and por_tracknextby='" + Session["UserID"].ToString() +"'" + query;
				conn.ExecuteQuery();
				FillGrid();
			}
			else
