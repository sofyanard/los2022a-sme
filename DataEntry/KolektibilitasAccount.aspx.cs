	...
	public partial class KolektibilitasAccount : System.Web.UI.Page
	{
		...
		void viewData()
		{
			...
			if (conn.GetRowCount()>0)
			{
				...
				try { DDL_LANCAR12.SelectedValue=conn.GetFieldValue("LANCAR_LAST_12BLN"); } catch {}
				try { DDL_FULLRECOVERY.SelectedValue=conn.GetFieldValue("FULL_RECOVERY"); } catch {}
				...
				try { DDL_COLL_KP_CURR.SelectedValue =conn.GetFieldValue("COLL_CURR_KP"); } catch {}
				try { DDL_COLL_MGM_CURR.SelectedValue =conn.GetFieldValue("COLL_CURR_MGM"); } catch {}
				try { DDL_WORSTIN12MONTH.SelectedValue = conn.GetFieldValue("COLL_W_12_B"); } catch {}
			...
			if(conn.GetRowCount() <= 0)
			{
				conn.QueryString = "select isnull(AP_BLBMPEMILIK,'') as AP_BLBMPEMILIK, " + 
					"isnull(AP_BLBMMGMT,'') as AP_BLBMMGMT, " + 
					"isnull(AP_BLBMUSAHA,'') as AP_BLBMUSAHA " + 
					" from APPLICATION where AP_REGNO = '" + Request.QueryString["curef"] + "C'";
				conn.ExecuteQuery();
			}
