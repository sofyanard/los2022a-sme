	...
	public partial class Jaminan_VEH : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				if (de != "1") conn.QueryString = "select DEALERID,DEALERID + ' - ' + DEALERDESC as DEALERDESC from RFDEALER ";
				else conn.QueryString = "select DEALERID,DEALERID + ' - ' + DEALERDESC as DEALERDESC from RFDEALER  where active='1'";

				conn.ExecuteQuery();
				for (int i=0; i< conn.GetRowCount(); i++)
					DDL_CL_DEALER.Items.Add(new ListItem(conn.GetFieldValue(i, 1), conn.GetFieldValue(i, 0)));

				if (de != "1")	conn.QueryString = "select CARTYPEID, CARTYPEDESC from RFCARTYPE ";
				else conn.QueryString = "select CARTYPEID, CARTYPEDESC from RFCARTYPE where active='1'";
