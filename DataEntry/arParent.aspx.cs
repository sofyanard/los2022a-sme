...
namespace SME.DataEntry
{
	...
	/// </summary>
	public partial class arParent : System.Web.UI.Page
	{
		...
		private void ViewListAspek()
		{
			...
			conn.QueryString = "select * from VW_ASPEK_LIST where AP_REGNO = '"+ REGNO +"'";
			conn.ExecuteQuery();
			...
			for (int i = 0; i < row; i++)
			{
				...
				{
					case "A":
						hlink = "arAspekTeknis.aspx";
						break;

					case "B":
						hlink = "arNonCashLoan.aspx";
						break;
				
					case "C":
						hlink = "arKontraktor.aspx";
						break;

					case "D":
						hlink = "arMiddleCommercial.aspx";
						break;
		...
		protected void btn_Insert_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				{
					case "1":
						if_Child.Attributes.Add("src", "arAspekTeknis.aspx?de=" + Request.QueryString["de"] + "&regno=" + REGNO + "&curef=" + CUREF + "&ketkredit=" + KETKREDIT + "&mc=" + MC + "&tc=" + TC + "&par=" + PAR);
						break;

					case "2":
						if_Child.Attributes.Add("src", "arNonCashLoan.aspx?de=" + Request.QueryString["de"] + "&regno=" + REGNO + "&curef=" + CUREF + "&ketkredit=" + KETKREDIT + "&mc=" + MC + "&tc=" + TC + "&par=" + PAR);
						break;
				
					case "3":
						if_Child.Attributes.Add("src", "arKontraktor.aspx?de=" + Request.QueryString["de"] + "&regno=" + REGNO + "&curef=" + CUREF + "&ketkredit=" + KETKREDIT + "&mc=" + MC + "&tc=" + TC + "&par=" + PAR);
						break;

					case "4":
						if_Child.Attributes.Add("src", "arMiddleCommercial.aspx?de=" + Request.QueryString["de"] + "&regno=" + REGNO + "&curef=" + CUREF + "&ketkredit=" + KETKREDIT + "&mc=" + MC + "&tc=" + TC + "&par=" + PAR);
						break;
