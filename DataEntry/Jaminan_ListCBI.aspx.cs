...
namespace SME.DataEntry
{
	...
	/// </summary>
	public partial class Jaminan_ListCBI : System.Web.UI.Page
	{
			...
			for (int i = 0; i < row; i++)
			{
				...
				*/
				...
				if (Request.QueryString["new"] != null && Request.QueryString["new"] == "1")
					t.NavigateUrl = "collateralnew.aspx?regno="+regno+"&curef="+ curef +"&coltypeid="+ cl_type +"&CL_SEQ="+ cl_seq + "&de=1" + "&new=" + Request.QueryString["new"];
				else
					...
					t.NavigateUrl = conn.GetFieldValue(i, 3) +".aspx?regno="+regno+"&curef="+ curef +"&coltypeid="+ cl_type +"&CL_SEQ="+ cl_seq + "&de=1";
				//------------------------------
				t.Target = "coldetail";
		...
		protected void BTN_DELETE_Click(object sender, System.EventArgs e)
		{
			...
			for (int i=0; i<row; i++)
			{
				if (((CheckBox)Table_List.FindControl("check"+ i)).Checked == true)
				{
					...
					string theregno = "";
					string jum = conn.GetFieldValue(0,0).ToString();

					if(conn.GetFieldValue(0,0).ToString() == "1")
					{
						conn.QueryString = "select AP_REGNO from LISTCOLLATERAL lc "+
							"left join COLLATERAL cl on lc.CU_REF = cl.CU_REF and lc.CL_SEQ = cl.CL_SEQ "+
							"left join RFCOLLATERALTYPE rf on cl.CL_TYPE = rf.COLTYPESEQ "+
							"where lc.cu_ref='"+Request.QueryString["curef"]+"' and lc.cl_seq = "+CL_SEQdel+" and AP_REGNO = '"+Request.QueryString["regno"]+"'";
						conn.ExecuteQuery();

						theregno = conn.GetFieldValue("AP_REGNO");
					}

					if (jum != "0" && theregno != Request.QueryString["curef"] + "C")
					{
