...
{
	/// <summary>
	/// Summary description for LinkDataUpload.
	/// </summary>
	public partial class LinkDataUpload : System.Web.UI.Page
	{
		protected Connection conn = new Connection(ConfigurationSettings.AppSettings["conn"]);
		private string theForm, theObj, theLink;
		...
		private void setTableItem()
		{
			...
			try
			{
				conn.QueryString = "SELECT * FROM VW_AP_LINK_DATA_UPLOAD WHERE [ID]='" + theLink + "'";
				conn.ExecuteQuery();
			}
			catch (Exception a)
			{
			...
			for (int i = 0; i < conn.GetRowCount(); i++)
			{
				...
				rdo.Name = "rdo";
				rdo.Value = conn.GetFieldValue(i,1).ToString();
				td.Controls.Add(rdo);
			...
			foreach (Control ctrl in Page.Controls)
			{
				if(ctrl is HtmlInputRadioButton)
				{
					...
					if(ctrlfix.Checked)
					{
						try
						{
							conn.QueryString = "SELECT * FROM VW_AP_LINK_DATA_UPLOAD WHERE [ID]='" + theLink + "' AND CODEID='" + ctrlfix.Value.ToString() + "'";
							conn.ExecuteQuery();

							//string codeid = conn.GetFieldValue("CODEID");
							string code = conn.GetFieldValue("CODE");

							Response.Write("<script language='JavaScript1.2'>window.opener.document." + 
								//theForm + "." + theObj + ".value='" + codeid + "-" + code + "'; " +
								theForm + "." + theObj + ".value='" + code + "'; " +
