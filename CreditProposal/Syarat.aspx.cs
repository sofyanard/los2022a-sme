	...
	public partial class Syarat : System.Web.UI.Page
	{
		...
		private void DatGrd_PK_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "Upload":
					Response.Write("<script language='javascript'>window.open('../CreditProposal/SyaratUploadFile.aspx?regno=" + Request.QueryString["regno"] + "&doctype=4&covseq=" + e.Item.Cells[1].Text + "&theForm=Form1&theObj=TXT_TEMP_PK','UploadDocument','status=no,scrollbars=yes,width=800,height=600');</script>");
					break;
			}
		}
		...
		private void DATGRID_LAIN_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "Upload":
					Response.Write("<script language='javascript'>window.open('../CreditProposal/SyaratUploadFile.aspx?regno=" + Request.QueryString["regno"] + "&doctype=6&covseq=" + e.Item.Cells[1].Text + "&theForm=Form1&theObj=TXT_TEMP_LAIN','UploadDocument','status=no,scrollbars=yes,width=800,height=600');</script>");
					break;
			}
		}
		...
		private void DATGRID_TERBIT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "Upload":
					Response.Write("<script language='javascript'>window.open('../CreditProposal/SyaratUploadFile.aspx?regno=" + Request.QueryString["regno"] + "&doctype=5&covseq=" + e.Item.Cells[1].Text + "&theForm=Form1&theObj=TXT_TEMP_TERBIT','UploadDocument','status=no,scrollbars=yes,width=800,height=600');</script>");
					break;
			}
		}
			...
			try 
			{
				...
				for (int i = 0; i < conn.GetRowCount(); i++) 
				{
					...
					if (conn.GetFieldValue(i, 3).Trim()!= "") 
					{
						...
						if (Request.QueryString["lmsreg"] != null)
							strtemp = strtemp + "&lmsreg=" + Request.QueryString["lmsreg"];
					}
					else 
					{
						strtemp = "";
