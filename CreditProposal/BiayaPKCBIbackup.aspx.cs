...
namespace SME.CreditProposal
{
	...
	/// </summary>
	public partial class BiayaPKCBIbackup : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				ViewList();
				conn.QueryString = "select top 1 PRODUCTID , APPTYPE, PROD_SEQ, AP_REGNO from VW_CREDITPROPOSAL_CREDLISTCBI "+
					"where CU_REF = '"+ LBL_CUREF.Text +"' order by apptype, productid ";
				conn.ExecuteQuery();
				if(conn.GetRowCount() > 0)
				{
					/*string autoLoadScript = "<script language='javascript'>" +
						"document.getElementById('frm_fasilitas').src='BiayaPK_DataCBI.aspx?regno=" +
					...
					string autoLoadScript = "<script language='javascript'>" +
						"document.getElementById('frm_fasilitas').src='BiayaPK_DataCBI.aspx?regno=" + conn.GetFieldValue(0, "AP_REGNO") + "&curef=" + LBL_CUREF.Text + "&tc=" + LBL_TC.Text + "&na=1" +
						"&productid=" + conn.GetFieldValue(0,0) + "&apptype=" +
						conn.GetFieldValue(0,1) + "&prod_seq=" + conn.GetFieldValue(0, "PROD_SEQ") + "';</script>";
					Page.RegisterStartupScript("LoadScript ", autoLoadScript);
				}
			}
		...
		private void ViewList()
		{
			conn.QueryString = "select distinct PRODUCTID , PRODUCTDESC , APPTYPE , APPTYPEDESC, PROD_SEQ, AP_REGNO "+
				"from VW_CREDITPROPOSAL_CREDLISTCBI "+
				"where CU_REF = '"+ LBL_CUREF.Text +"' order by apptype, productid ";
			conn.ExecuteQuery();
			...
			for (int i = 0; i < row; i++)
			{
				...
				t.NavigateUrl = "BiayaPK_DataCBI.aspx?regno=" + ap_regno + "&curef=" + LBL_CUREF.Text +
