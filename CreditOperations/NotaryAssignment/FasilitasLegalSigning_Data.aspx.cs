...
namespace SME.CreditOperations.NotaryAssignment
{
	...
	public partial class FasilitasLegalSigning_Data : System.Web.UI.Page
	{
            ...
            try
            {
                ...
                if (conn.GetRowCount() > 0)
                {
                    ...
                    //status = CreateWord(DDL_FORMAT_TYPE.SelectedValue);
                        status = client.DocumentExportASCXCreateWordPk(DDL_FORMAT_TYPE.SelectedValue, Request.QueryString["regno"], Request.QueryString["prod_seq"], Session["UserID"].ToString());
                    //else ((doctype == "EXCEL") && (acttype == "WRITE"))
		...
		private void DATA_EXPORT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					try
                    {
                        conn.QueryString = "SELECT TOP 1 UPLOAD_PATH FROM VW_DOCEXPORT_PARAMETER WHERE TEMPLATE_GROUP = '" + e.Item.Cells[0].Text + "' OR TEMPLATE_ID = '" + e.Item.Cells[0].Text + "'";
                        conn.ExecuteQuery();
                        ...
                        if (conn.GetRowCount() > 0)
                        {
                            ...
                            conn.QueryString = "EXEC DOCEXPORT_SAVE '2', '" + e.Item.Cells[0].Text + "','" + Request.QueryString["regno"] +
                                                "', '" + Session["UserID"] + "', '" + e.Item.Cells[1].Text + "'";
		...
		{
            DataTable dt = new DataTable();
            conn.QueryString = "SELECT * FROM VW_DOCEXPORT_VIEWFILEEXPORT WHERE AP_REGNO = '" + Request.QueryString["regno"] + "' AND TEMPLATE_GROUP = 'PK'";
            conn.ExecuteQuery();
            ...
            for (int i = 1; i <= DATA_EXPORT.Items.Count; i++)
            {
                ...
                LinkButton hpDelete = (LinkButton)DATA_EXPORT.Items[i - 1].Cells[3].FindControl("HL_DELETE");
                hpDownload.NavigateUrl = DATA_EXPORT.Items[i - 1].Cells[5].Text.Trim();
