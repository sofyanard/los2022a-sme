...
{
	/// <summary>
	/// Summary description for LegalProcessMonitoringList.
	/// </summary>
	public partial class LegalProcessMonitoringList : System.Web.UI.Page
	{
...
//				Response.Redirect("/SME/Restricted.aspx");
		...
		private void bindData()
		{
			...
			if (sqlCond != "") 
			{
				/*conn.QueryString = "SELECT * FROM VW_CREOPR_LEGALPROCMONITORING_LIST " + sqlCond +
					...
					Session["UserID"].ToString() + "') or br_ccobranch is null)"; // and branch_code='" + Session["BranchID"].ToString() + "'";
					*/
				conn.QueryString = "SELECT * FROM VW_CREOPR_LEGALPROCMONITORING_LIST " + sqlCond +
					" and AP_CURRTRACK='" + Request.QueryString["tc"] + "' and (AP_CO='" +
					Session["UserID"].ToString() + "' or AP_CO is null)"; // and branch_code='" + Session["BranchID"].ToString() + "'";
				conn.ExecuteQuery();
			...
			{
				conn.QueryString = "SELECT * FROM VW_CREOPR_LEGALPROCMONITORING_LIST WHERE " +
					"AP_CURRTRACK='" + Request.QueryString["tc"] + "' and (AP_CO='" +
					Session["UserID"].ToString() + "' or AP_CO is null)";
			...
			conn.QueryString = "SELECT AP_REGNO, CU_REF, NAME, AP_SIGNDATE, AP_RELMNGR, LIMIT "+
				"FROM VW_CREOPR_LEGALPROCMONITORING_LIST ";
			conn.ExecuteQuery();
		...
		private void DataGrid1_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "view":
					...
					Response.Redirect("LegalProcessMonitoringDetail.aspx?regno="+ e.Item.Cells[0].Text +"&curef="+ e.Item.Cells[1].Text +"&tc="+ LBL_H_TC.Text+"&mc=" + Request.QueryString["mc"]);
					break;
