...
{
	/// <summary>
	/// Summary description for Jenis Agunan
	/// </summary>
	public partial class RFAgunanParam : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack) 
			{
				...
				LBL_ID.Text = "AGUNANID";
				LBL_DESC.Text = "AGUNANDESC";
		...
		{
			conn.QueryString = "select * from VW_RFCOLLATERALTYPE";
			conn.ExecuteQuery();

			DDL_COLTYPEID.Items.Add(new ListItem("- PILIH -",""));
			for(int i=0; i<conn.GetRowCount(); i++) 
			{
				DDL_COLTYPEID.Items.Add(new ListItem(conn.GetFieldValue(i, "COLTYPEDESC"), conn.GetFieldValue(i, "COLTYPESEQ")));
			}
		}
		...
		private void viewPendingData() 
		{
			...
			//conn.QueryString = "select * from PENDING_RFJENISAGUNAN";
			conn.QueryString = "select * from VW_PARAM_GENERAL_PENDING_RFJENISAGUNAN";
			conn.ExecuteQuery();
			...
			{
				dr = dt.NewRow();
				dr[0] = conn.GetFieldValue(i, "AGUNANID");
				dr[1] = conn.GetFieldValue(i, "AGUNANDESC");
				dr[2] = conn.GetFieldValue(i, "COLTYPEID");
		...
		{
			if (LBL_ACTIVE.Text.Trim() == "1")
				//conn.QueryString = "select * from RFJENISAGUNAN where ACTIVE = '1'";
				conn.QueryString = "select * from VW_PARAM_GENERAL_RFJENISAGUNAN where ACTIVE = '1'";
			else
				//conn.QueryString = "select * from RFJENISAGUNAN";
				conn.QueryString = "select * from VW_PARAM_GENERAL_RFJENISAGUNAN";
			conn.ExecuteQuery();
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			{
				conn.QueryString = "select * from RFJENISAGUNAN WHERE AGUNANID ='" + TXT_ID.Text.Trim() + "'";
				conn.ExecuteQuery();
		...
		{			
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				...
				case "delete":
					...
					conn.QueryString = "delete from PENDING_RFJENISAGUNAN WHERE AGUNANID='" + id + "'";
					conn.ExecuteQuery();
					viewPendingData();
					break;
		...
		{
			conn.QueryString = "exec PARAM_GENERAL_RFJENISAGUNAN_MAKER '" + pendingStatus + 
								"','" + id + 
								"','" + desc + 
								"','" + coltypeid + "'";
			try 
