...
{
	/// <summary>
	/// Summary description for Kelurahan
	/// 
	/// TODO : Validasi untuk screen
	/// 
	/// </summary>
	public partial class RFZipCodeCityParam : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from VW_RFCITY";
			conn.ExecuteQuery();

			DDL_COLTYPEID.Items.Add(new ListItem("- PILIH -",""));
			for(int i=0; i<conn.GetRowCount(); i++) 
			{
				DDL_COLTYPEID.Items.Add(new ListItem(conn.GetFieldValue(i, "CITYNAME"), conn.GetFieldValue(i, "CITYID")));
			}
		}
		...
		private void viewPendingData(string CITYID) 
		{
			string pendCol = "";

			conn.QueryString = "select * from VW_PARAM_GENERAL_PENDING_RFZIPCODECITY where CITYID = '"+ CITYID +"'";
			conn.ExecuteQuery();
			...
			{
				dr = dt.NewRow();
				dr[0] = conn.GetFieldValue(i, "ZIPCODE");
				dr[1] = conn.GetFieldValue(i, "DESCRIPTION");
				dr[2] = conn.GetFieldValue(i, "CITYID");
				dr[3] = conn.GetFieldValue(i, "CITYNAME");
		...
		private void viewExistingData(string CITYID) 
		{
			if (LBL_ACTIVE.Text.Trim() == "1")
				conn.QueryString = "select * from VW_PARAM_GENERAL_RFZIPCODECITY where ACTIVE = '1' and CITYID = '"+ CITYID +"'";
			else
				conn.QueryString = "select * from VW_PARAM_GENERAL_RFZIPCODECITY where CITYID = '"+ CITYID +"'";
			conn.ExecuteQuery();
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			{
				conn.QueryString = "select * from RFZIPCODECITY WHERE ZIPCODE ='" + TXT_ID.Text.Trim() + "'";
				conn.ExecuteQuery();
		...
		{			
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				...
				case "delete":
					...
					conn.QueryString = "delete from PENDING_RFZIPCODECITY WHERE ZIPCODE ='" + id + "'";
					conn.ExecuteQuery();
					viewPendingData(DDL_COLTYPEID.SelectedValue);
					break;
		...
		private void executeMaker(string id, string desc, string id2, string pendingStatus) 
		{
			conn.QueryString = "exec PARAM_GENERAL_RFZIPCODECITY_MAKER '" + pendingStatus + 
								"','" + id + 
								"','" + desc + 
								"','" + id2 + "'";
			try 
