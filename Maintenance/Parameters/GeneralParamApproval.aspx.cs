...
{
	/// <summary>
	/// Summary description for GeneralParamApproval.
	/// </summary>
	public partial class GeneralParamApproval : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SELECT PARAMNAME,PARAMLINK FROM RFPARAMETER WHERE ISMAKER='0' AND PG_ID='1' ORDER BY PARAMNAME";
			conn.ExecuteQuery();
			...
			for (int i = 0; i < conn.GetRowCount(); i++)
			{
				...
				if (conn.GetFieldValue(i,1).IndexOf("?") < 0)
					...
					link.NavigateUrl = conn.GetFieldValue(i,1) + "&mc=" + Request.QueryString["mc"];
				//link.NavigateUrl = conn.GetFieldValue(i,1);	//---- old version
