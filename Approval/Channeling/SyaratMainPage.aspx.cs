	...
	public partial class SyaratMainPage : System.Web.UI.Page
	{
		...
		private void DisableAllControl()
		{
			...
			if(conn.GetRowCount() == 0)
			{
				...
				BTNINSERT_PK.Visible = false;
				BTNINSERT_TERBIT.Visible = false;
				BTNINSERT_LAIN.Visible = false;

				for (int i = 1; i <= DatGrd_PK.Items.Count; i++)
				{
					DatGrd_PK.Items[i-1].Cells[7].Visible = false;
					DatGrd_PK.Items[i-1].Cells[6].Visible = false;
					DataGrid dgpkdoc = (DataGrid) DatGrd_PK.Items[i-1].Cells[1].FindControl("DG_PKDOC");
					for (int j = 0; j < dgpkdoc.Items.Count; j++)
					{
						LinkButton HpDownload = (LinkButton) dgpkdoc.Items[j].Cells[4].FindControl("LB_DELETE1");
						HpDownload.Visible = false;
					}
				}
				
				for (int i = 1; i <= DATGRID_TERBIT.Items.Count; i++)
				{
					DATGRID_TERBIT.Items[i-1].Cells[7].Visible = false;
					DATGRID_TERBIT.Items[i-1].Cells[6].Visible = false;
					DataGrid dgpkdoc = (DataGrid) DATGRID_TERBIT.Items[i-1].Cells[1].FindControl("DG_TERBITDOC");
					for (int j = 0; j < dgpkdoc.Items.Count; j++)
					{
						LinkButton HpDownload = (LinkButton) dgpkdoc.Items[j].Cells[4].FindControl("LB_DELETE2");
						HpDownload.Visible = false;
					}
				}

				for (int i = 1; i <= DATGRID_LAIN.Items.Count; i++)
				{
					DATGRID_LAIN.Items[i-1].Cells[7].Visible = false;
					DATGRID_LAIN.Items[i-1].Cells[6].Visible = false;
					DataGrid dgpkdoc = (DataGrid) DATGRID_LAIN.Items[i-1].Cells[1].FindControl("DG_LAINDOC");
					for (int j = 0; j < dgpkdoc.Items.Count; j++)
					{
						LinkButton HpDownload = (LinkButton) dgpkdoc.Items[j].Cells[4].FindControl("LB_DELETE3");
						HpDownload.Visible = false;
					}
				}
			}
