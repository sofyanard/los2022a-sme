...
namespace SME.MAS.CollateralAdministration.Reporting
{
	/// <summary>
	/// Summary description for CollateralAdministrationReport.
	/// </summary>
	public partial class CollateralAdministrationReport : System.Web.UI.Page
	{
		protected Tools tool = new Tools();
		protected Connection conn = new Connection(ConfigurationSettings.AppSettings["conn"]);
		protected Connection conn2 = new Connection(ConfigurationSettings.AppSettings["conn"]);
		string vargrouptemplate;
		string templateid;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				vargrouptemplate = "MASCOLLATERAL";
				templateid = "MASCOLLATERAL";

				string qryddl = "SELECT DISTINCT TEMPLATE_ID, TEMPLATE_DESC FROM MAS_DOCEXPORT_TEMPLATE_MASTER WHERE TEMPLATE_GROUP = '" + vargrouptemplate + "'";;
		...
		private void ViewScreenMenu()
		{
			...
			h2.Font.Bold = true;
			h2.NavigateUrl = "CollateralAdministrationReport.aspx";

			HyperLink h3 = new HyperLink();
			h3.Text = "Document Report";
			h3.Font.Bold = true;			
			h3.NavigateUrl="../DetailCollateral/DataJaminan.aspx";			
		...
		{    
			...
			this.DATA_EXPORT.ItemDataBound += new System.Web.UI.WebControls.DataGridItemEventHandler(this.DATA_EXPORT_ItemDataBound);
		...
		private void ViewExportFiles()
		{
			...
			conn.QueryString = "SELECT * FROM VW_MAS_DOCEXPORT_VIEWFILEEXPORT WHERE TEMPLATE_GROUP = 'MASCOLLATERAL' AND FE_USERID = '" + Session["UserID"].ToString() + "'";
			conn.ExecuteQuery();
		...
		private string CreateExcel2(string templateid)
		{
			...
			conn.QueryString = "SELECT TOP 1 * FROM VW_MAS_DOCEXPORT_PARAMETER WHERE TEMPLATE_ID = 'MASCOLLATERAL'";
			conn.ExecuteQuery();
			...
			if (conn.GetRowCount() > 0)
			{
				...
				try
				{
					...
					conn.QueryString = "SELECT SHEET_ID, SHEET_SEQ, STOREDPROCEDURE FROM MAS_DOCEXPORT_TEMPLATE_MASTER WHERE TEMPLATE_ID = 'MASCOLLATERAL'";
					conn.ExecuteQuery();
					...
					if (dt1.Rows.Count > 0)
					{
						for (int i = 0; i < dt1.Rows.Count; i++)
						{
							...
							if (dt3.Rows.Count > 0)
							{
								...
								conn.QueryString = "SELECT CELL_ROW, CELL_COL, DB_FIELD FROM MAS_DOCEXPORT_TEMPLATE_DETAIL WHERE TEMPLATE_ID = 'MASCOLLATERAL' AND SHEET_ID = '" + sheetid + "' AND SHEET_SEQ = '" + sheetseq + "' ORDER BY SEQ";
								conn.ExecuteQuery();
		...
		protected void FillDDLUnit(string distcode)
		{
			...
			conn2.ExecuteQuery();
			for (int i = 0; i < conn2.GetRowCount(); i++)
				DDL_UNIT.Items.Add(new ListItem(conn2.GetFieldValue(i,1),conn2.GetFieldValue(i,0)));	
		}
		...
		{
			FillDDLCluster(DDL_DISTRIK.SelectedValue);			
		}

		protected void DDL_CLUSTER_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			FillDDLUnit(DDL_CLUSTER.SelectedValue);					
		}

		private void DATA_EXPORT_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if ( e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item ) 
			{
				System.Web.UI.WebControls.LinkButton linkntn=(System.Web.UI.WebControls.LinkButton)e.Item.FindControl("FE_DELETE");
