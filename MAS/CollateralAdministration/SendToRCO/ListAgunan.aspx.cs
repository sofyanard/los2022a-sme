...
{
	/// <summary>
	/// Summary description for ListAgunan.
	/// </summary>
	public partial class ListAgunan : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from VW_MAS_INPUT_NEW_COLLATERAL where trackcode = 'M1.4' and CA_NAME = '" + Session["UserID"].ToString() + "' and ACC_STATUS = '1' and agunan_status <> '3'";
			conn.ExecuteQuery();
		...
		private void DGR_RESULT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				case "View":					
					Response.Redirect("SendToRCO.aspx?sta=exist&acc_number=" + e.Item.Cells[0].Text + "&collateral_id=" + e.Item.Cells[2].Text + 
						"&tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"]+ 
						"&type=" + e.Item.Cells[3].Text + "&acc_status=" + e.Item.Cells[4].Text);
					break;
			}
		}
