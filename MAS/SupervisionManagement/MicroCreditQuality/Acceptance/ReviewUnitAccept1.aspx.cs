...
{
	/// <summary>
	/// Summary description for ReviewUnitAccept1.
	/// </summary>
	public partial class ReviewUnitAccept1 : System.Web.UI.Page
	{
		...
		protected void BTN_SEND_Click(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "select pic_input from VW_MAS_RF_UNIT_REVIEW where unit_seq#='"+Request.QueryString["seq_unit"]+"' ";
			conn.ExecuteQuery();
			pic_input = conn.GetFieldValue("pic_input");

			conn.QueryString = "exec MAS_MICRO_CREDIT_TRACKUPDATE '" +
				Request.QueryString["seq_unit"] + "' , 'M2.3.3' , '" + 
				Session["UserID"].ToString() + "' , '"+ pic_input +"' ";
			conn.ExecuteQuery();
