...
namespace SME.CustomerInfo
{
	...
	public partial class COLLATERAL_LC : System.Web.UI.Page
	{
		...
		private void ViewData()
		{
			...
			TXT_CL_ISSUEBANK.Text = conn.GetFieldValue("CL_ISSUEBANK");
		...
		private void Update()
		{
			conn.QueryString = "exec DE_COLL_LC '"+ LBL_CUREF.Text +"', "+ LBL_CL_SEQ.Text +", 0, "+
				...
				TXT_CL_ISSUEBANK.Text +"', '"+ TXT_CL_ISSUENAME.Text +"', '"+ TXT_CL_CONDITION.Text +"', '"+ 
				TXT_CL_COMPNAME.Text +"', "+ tool.ConvertFloat(TXT_CL_GUARANTEEVAL.Text);	
			conn.ExecuteNonQuery();
			ViewData();
