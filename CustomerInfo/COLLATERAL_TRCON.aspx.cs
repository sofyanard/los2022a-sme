	...
	public partial class COLLATERAL_TRCON : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			{				
				...
				conn.QueryString = "select DEALERID, DEALERID + ' - ' + DEALERDESC from RFDEALER  where active='1'";
				conn.ExecuteQuery();
				for (int i=0; i< conn.GetRowCount(); i++)
		...
		private void Update()
		{
			conn.QueryString = "exec DE_COLL_TRCON '"+ LBL_CUREF.Text +"', "+ LBL_CL_SEQ.Text +", 0, "+
				...
				tool.ConvertNull(DDL_CL_DEALER.SelectedValue) +", "+ tool.ConvertFloat(TXT_CL_GUARANTEEVAL.Text);
			conn.ExecuteNonQuery();
			ViewData();
