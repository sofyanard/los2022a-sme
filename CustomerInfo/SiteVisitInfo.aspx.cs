...
namespace SME.CustomerInfo
{
	...
	public partial class SiteVisitInfo : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_CUSTINFO_SITEVISITINFO WHERE CU_REF = '" + Request.QueryString["curef"] + "'";
			conn.ExecuteQuery();
		...
		private void DG_SITEVISIT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				case "view":
					string url = e.Item.Cells[5].Text + "&mc=" + Request.QueryString["mc"] + "&scr=" + Request.QueryString["scr"] + "&lkkn=" + Request.QueryString["lkkn"];
		...
		protected void BTN_NEW_Click(object sender, System.EventArgs e)
		{
			try
			{
				conn.QueryString = "exec CUSTINFO_SITEVISITINFO_NEWVISIT '" + 
					Request.QueryString["curef"] + "', '" +
					Session["UserID"].ToString() + "'";
				conn.ExecuteQuery();

				string url = conn.GetFieldValue(0,0) + "&curef=" + Request.QueryString["curef"] + "&mc=" + Request.QueryString["mc"] + "&scr=" + Request.QueryString["scr"];
				Response.Redirect(url);
			}
			catch (Exception ex)
			{
