...
namespace SME.InitialDataEntry
{
	...
	/// </summary>
	public partial class InfoCustomer : System.Web.UI.Page
	{
...
//			Menu.Controls.Add(new LiteralControl("&nbsp;&nbsp;&nbsp;"));
				...
				for (int i = 0; i < conn.GetRowCount(); i++) 
				{
					...
					if (conn.GetFieldValue(i, 3).Trim()!= "") 
					{
						if (conn.GetFieldValue(i,3).IndexOf("mc=") >= 0)
							strtemp = "regno=" + Request.QueryString["regno"] + "&curef="+Request.QueryString["curef"]+"&tc="+Request.QueryString["tc"]+"&sta=" + Request.QueryString["sta"];
						else	
							strtemp = "regno=" + Request.QueryString["regno"] + "&curef="+Request.QueryString["curef"]+"&mc="+Request.QueryString["mc"]+"&tc="+Request.QueryString["tc"]+"&sta=" + Request.QueryString["sta"];
					}
					else 
					{
						strtemp = "";
		...
		protected void BTN_SEARCHCOMP_Click(object sender, System.EventArgs e)
		{
            ...
            Response.Write("<script language='javascript'>window.open('../InitialDataEntry/SearchZipcode.aspx?targetFormID=Form1&targetObjectID=TXT_CU_COMPZIPCODE&trgObjID2=TXT_CU_COMPADDR3&trgObjID3=TXT_CU_COMPCITY&trgObjID4=LBL_CU_COMPCITY','SearchZipcode','status=no,scrollbars=yes,width=640,height=480');</script>");
		}
...
*/
		...
		/*
		private void CB_WASPADA_CheckedChanged(object sender, System.EventArgs e)
		{
			if (CB_WASPADA.Checked == true)
			{
				//TXT_CU_CIF_P.CssClass = "";
