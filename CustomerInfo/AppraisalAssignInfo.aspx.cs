...
{
	/// <summary>
	/// Summary description for AppraisalAssignInfo.
	/// </summary>
	public partial class AppraisalAssignInfo : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_CUSTINFO_APPRAISALASSIGNINFO WHERE CU_REF = '" + Request.QueryString["curef"] + "'";
			conn.ExecuteQuery();
		...
		private void DG_HIST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete":
					try
					{
						conn.QueryString = "EXEC CUSTINFO_APPRAISALASSIGNINFO_DELETE '" +
							e.Item.Cells[0].Text + "'";
		...
		protected void BTN_NEW_Click(object sender, System.EventArgs e)
		{
			try
			{
				conn.QueryString = "exec CUSTINFO_APPRAISALASSIGNINFO_NEW '" + 
