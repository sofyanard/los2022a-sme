...
{
	/// <summary>
	/// Summary description for AssigmentList.
	/// </summary>
...
*/
		...
		private string getQueryString(string tc) 
		{
			...
			if (tc=="" || tc==null || tc=="&nbsp;") 
			{
				queryString = VIEWNAME + " where BR_CCOBRANCH = '" + LBL_BR_CCOBRANCH.Text + "'" +
		...
		private void viewAllData() 
		{
			...
			dt.Columns.Add(new DataColumn("CP_LIMIT"));
			...
			for (int i = 0; i < DataGrid1.Items.Count; i++) 
			{
				...
				//tool.FormatDate(DataGrid1.Items[i].Cells[3].Text, true);


				isComplete = false;
				//--- Status Assignment Officer
				if (DataGrid1.Items[i].Cells[7].Text == "" || DataGrid1.Items[i].Cells[7].Text == "&nbsp;" || DataGrid1.Items[i].Cells[7].Text == "0") 
				{
				...
				else
				{
					...
					if (DataGrid1.Items[i].Cells[7].Text == "2" && DataGrid1.Items[i].Cells[8].Text == "1") 
					{
				...
				if (!isComplete) 
				{
					...
					if (DataGrid1.Items[i].Cells[8].Text == "2") 
					{
						LBL_BS_COMPLETE.Text = "Yes";
						IMG_BS_COMPLETE.ImageUrl = "../image/Complete.gif";
					}
					else if (DataGrid1.Items[i].Cells[8].Text == "1") 
					{
		...
		private void DataGrid1_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "Continue":
					...
					Response.Redirect("Assigment.aspx?regno=" + regno + "&curef="+curef + "&tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"] + "&isassign=" + BS_BIASSIGN + "&iscomplete=" + BS_COMPLETE);
					break;
			}
		}
...
/*
		...
		{	
			...
			conn.QueryString = "exec IT_LIST_COMPLIANCE '"+txt_regno.Text+"', '"+lbl_userid.Text+"'";
			conn.ExecuteQuery();
