...
{
	/// <summary>
	/// Summary description for Assigment.
	/// </summary>
	public partial class Assigment : System.Web.UI.Page
	{
...
*/
		private void fillPetugas() 
		{
			...
			GlobalTools.fillRefList(DDL_PETUGAS, "select * from VW_ASSIGNMENT_USER where SU_UPLINER = '" + userID + "' OR SU_MIDUPLINER = '" + userID + "' OR SU_CORUPLINER = '" + userID + "'",false,conn); 
				//+ 
...
*/
		...
		protected void BTN_ASSIGN_Click(object sender, System.EventArgs e)
		{
			...
			//--- Assign petugas
			conn.QueryString = getQuery(LBL_TC.Text) + " '" + LBL_AP_REGNO.Text+"','"+LBL_CU_REF.Text+"', '" + DDL_PETUGAS.SelectedValue + "', '1'";
			...
			//Response.Redirect("AssigmentList.aspx?mc="+Request.QueryString["mc"]+"&tc="+Request.QueryString["tc"]);
		}
...
*/
		...
		{
			Response.Redirect("/SME/ITTP/AssigmentList.aspx?tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"]);
			//Response.Redirect("/SME/" + DMS.CuBESCore.Logic.backLink(Request.QueryString["tc"].ToString(), conn));

		}
/*
		private void BTN_ASSIGN_TEAM_Click(object sender, System.EventArgs e)
		{
		...
		private void BTN_RECHECK_Click(object sender, System.EventArgs e)
		{
			try 
			{
