...
{
	/// <summary>
	/// Summary description for Process1.
	/// </summary>
	...
	/*
		...
		{    
			...
			this.RDO_PBARU.SelectedIndexChanged += new System.EventHandler(this.RDO_PBARU_SelectedIndexChanged_1);
			this.DDL_CHANNCOMP.SelectedIndexChanged += new System.EventHandler(this.DDL_CHANNCOMP_SelectedIndexChanged);
			this.DDL_AANO.SelectedIndexChanged += new System.EventHandler(this.DDL_AANO_SelectedIndexChanged);
			this.DDL_PRODUCTID.SelectedIndexChanged += new System.EventHandler(this.DDL_FACILITY_CODE_SelectedIndexChanged);
			this.DDL_ACC_SEQ.SelectedIndexChanged += new System.EventHandler(this.DDL_ACC_SEQ_SelectedIndexChanged);
			this.DDL_ACC_NO.SelectedIndexChanged += new System.EventHandler(this.DDL_ACC_NO_SelectedIndexChanged);
			this.BTN_ADD.Click += new System.EventHandler(this.BTN_ADD_Click);
			this.BTN_CANCEL.Click += new System.EventHandler(this.BTN_CANCEL_Click);
			this.BTN_CANCEL_ADD.Click += new System.EventHandler(this.BTN_CANCEL_ADD_Click);
			this.DDL_NCLPROD.SelectedIndexChanged += new System.EventHandler(this.DDL_NCLPROD_SelectedIndexChanged);
...
/*
			...
			if (RDO_PBARU.SelectedValue == "1")
				creddetail.Attributes.Add("src", "PermohonanBaru2.aspx?regno=" + regno + "&curef=" + curef + "&prog=" + prog + "&tc=" + tc + "&mc=" + mc + "&ket_code=" + conn.GetFieldValue(0,"KET_CODE") + "&mainregno=" + mainregno + "&mainprod_seq="+ mainprod_seq +"&mainproductid="+ mainproductid);
...
/*
		...
		private void cekKetKreditForEmptyLoan() 
		{
			try 
			{
				conn.QueryString = "exec IT_IDE_KETIT_NOLOAN '" + LBL_REGNO.Text + "'";
				conn.ExecuteNonQuery();
			}
			catch (NullReferenceException) 
			{
...
/*
		...
		private void viewData() 
		{
			conn.QueryString = "select * " + 
				"from VW_KET_IT where AP_REGNO = '" + LBL_REGNO.Text + "'";
			conn.ExecuteQuery();			

			DATAGRID1.DataSource = conn.GetDataTable().DefaultView;
			try 
			{
				DATAGRID1.DataBind();
			} 
			...
			{			
				LinkButton lb = (LinkButton) DATAGRID1.Items[i].Cells[4].FindControl("LNK_ADD");
				string ketCode = DATAGRID1.Items[i].Cells[0].Text.Trim();
				conn.QueryString = "select apptype from custproduct where ap_regno ='"+LBL_AP_REGNO.Text.Trim()+"' and ket_code = '"+ketCode+"'";
				conn.ExecuteQuery();
				if(conn.GetFieldValue("apptype").Trim() == "01") lb.Visible = false;			
			}
		...
		private void RDO_PBARU_SelectedIndexChanged_1(object sender, System.EventArgs e)
		{
			DDL_CHANNCOMP.Enabled = false;
...
//		}
/*
		private void DDL_FACILITY_CODE_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			string _CUREF = "";
			if (RDO_PBARU.SelectedValue == "2") _CUREF = DDL_CHANNCOMP.SelectedValue;
			else _CUREF = LBL_CUREF.Text;
		...
		private void BTN_UPDATE_STATUS_Click(object sender, System.EventArgs e)
		{
...
/*			} 
		...
		private void DDL_ACC_NO_SelectedIndexChanged(object sender, System.EventArgs e)
		{
...
/*
		...
		private void BTN_CANCEL_ADD_Click(object sender, System.EventArgs e)
		{
			//creddetail.Attributes.Add("src","");
			TXT_KETKREDIT_DESC.Text = "";
			TBL_DETAIL.Visible = false;
		...
		private void DDL_CHANNCOMP_SelectedIndexChanged(object sender, System.EventArgs e)
		{
		...
		private void DDL_NCLPROD_SelectedIndexChanged(object sender, System.EventArgs e)
		{
