...
{
	/// <summary>
	/// Summary description for ComplianceCondition.
	/// </summary>
	public partial class ComplianceCondition : System.Web.UI.Page
	{
...
*/
		private void ViewProses()
		{
			...
			conn.QueryString = "select seq, des from vw_it_syarat where (sy_status='0' or sy_status is null or sy_status='') and ap_regno='"+LBL_REGNO.Text+"'";
			conn.ExecuteQuery();
...
/*
		private void ViewDataPKDoc()
		{
			for (int i=0;i<DGR_LIST.Items.Count;i++)
			{
				...
				conn.QueryString = "SELECT * FROM VW_IT_SYARAT_DOC WHERE AP_REGNO = '" + Request.QueryString["regno"] + "' AND COVSEQ = '" + DGR_LIST.Items[i].Cells[3].Text.Trim() + "'";
				conn.ExecuteQuery();
...
*/
		...
		protected void BTN_DF_INPUT_Click(object sender, System.EventArgs e)
		{
			...
			if (!DDL_PROSES.SelectedValue.Equals(""))
			{
				try
				{
					conn.QueryString = "exec IT_SP_SYARAT '"+ LBL_REGNO.Text +"', "+DDL_PROSES.SelectedValue+", "+
						"'1', "+tools.ConvertDate(TXT_TGL.Text, DDL_BLN.SelectedValue, TXT_THN.Text )+", '"+ TXT_KET.Text +"','1'";
		...
		private void DGR_LIST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					try
					{
						conn.QueryString = "exec IT_SP_SYARAT '"+ LBL_REGNO.Text +"', "+Int16.Parse(e.Item.Cells[3].Text)+", "+
							"'0', "+tools.ConvertDate(1.ToString(),1.ToString(),2000.ToString())+", '"+ e.Item.Cells[2].Text +"','1' ";
						conn.ExecuteNonQuery();
					}
					catch (Exception ex)
					{
...
*/			}		
	...
	/*	
			...
			{
				if (dt.Rows[i]["APPTYPE"].ToString() == "T02")
				{
		...
		*/
			...
			*/			
			...
			conn.QueryString = "select APPTYPE, productid from approval_decision where ap_regno='"+LBL_REGNO.Text+"' and prod_seq='"+LBL_PROD_SEQ.Text+"'";
			conn.ExecuteQuery();
			LBL_APPTYPE.Text = conn.GetFieldValue("APPTYPE");
			LBL_PRODUCTID.Text = conn.GetFieldValue("productid");

			//string pk = RBL_LEGALSTA.SelectedValue.Trim();
			conn.QueryString = "exec IT_LGL_SCREDIT '"+ 
				LBL_REGNO.Text +"', '"+ 
				LBL_APPTYPE.Text +"', '" +
				LBL_PRODUCTID.Text + "', '" + 
			...
			Response.Redirect("ComplianceList.aspx?tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"] + "&msg=" + msg);
		...
		private string getNextStepMsg(string regno, string tc) 
		{
			...
			try 
			{
				 ...
				 ***/
				conn.QueryString  = "exec IT_TRACKNEXTMSG1 '" + regno + "', '" + tc + "'";
				conn.ExecuteQuery();
...
*/
		...
		{
			Response.Write("<script for=window event=onload language='javascript'>PopupPage('AcqInfo.aspx?regno=" + LBL_REGNO.Text + "&curef=" + TXT_CU_REF.Text + "&aprv=CO&theForm=Form1&theObj=TXT_TEMP', '800','300');</script>");
...
*/			////////////////////////////////////////////////////////////////////////////////////////////
		...
		protected void TXT_TEMP_TextChanged(object sender, EventArgs e)
		{
			if (TXT_TEMP.Text != "") 
			{
				...
				Response.Redirect("ComplianceList.aspx?tc=" + Request.QueryString["tc"] + "&mc=" + Request.QueryString["mc"]);
			}
		}
		...
		private void DGR_LIST_ItemCommand_1(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
...
/*
		private void TXT_TEMP_PK_TextChanged(object sender, System.EventArgs e)
		{
			ViewDGR_LIST();
			SecureData();
			TXT_TEMP_PK.Text = "";
...
*/
/*		private void BTN_SAVEREV_Click(object sender, System.EventArgs e)
		{
