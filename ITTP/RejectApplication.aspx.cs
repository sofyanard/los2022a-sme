...
namespace SME.ITTP
{
	...
	public partial class RejectApplication : System.Web.UI.Page
	{
		...
		private void ViewGrid()
		{
			...
			for (int i = 0; j < conn.GetRowCount(); i++) 
			{
				...
				if (conn.GetFieldValue(j, "TRACK")=="3.6")
				{
					...
					t.Text			= "view";					
					t.NavigateUrl	= "RejectApplication.aspx?tampil=true&ap_link="+conn.GetFieldValue(j, "AP_REGNO")+"&"+link + "&mc=" + Request.QueryString["mc"];
					oTable.Rows[i+1].Cells.Add(new TableCell());
		...
		{
			Response.Redirect("RejectApplication.aspx?ap_regno=&nama=&cust=&tgl=&bln=&thn=&noID=&area=&cabang=&tampil=&ap_link=&mc=" + Request.QueryString["mc"]);
		}
			...
			if (oTable1.Visible==true)		
			{
				...
				try 
				{
					...
					for (int i = 0; i<conn.GetRowCount(); i++)
					{
						...
						{
							conn.QueryString = "exec IT_ENTRY_REJECT_CANCEL "+
