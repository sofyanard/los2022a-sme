...
{
	/// <summary>
	/// Summary description for AssignBulkCust.
	/// </summary>
	public partial class AssignBulkCust : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_ASSIGNMENTBULK_LISTCUST WHERE USERID = '" + Request.QueryString["uid"] + "' " + LBL_SORT.Text;
			conn.ExecuteQuery();
		...
		protected void BTN_ASSIGN_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				for (int i=0; i<DatGrd.Items.Count; i++)
				{
					...
					{
						conn.QueryString = "EXEC ASSIGNMENTBULK_PROCESSCUST '" +
		...
		protected void BTN_ASSIGNALL_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				conn.QueryString = "EXEC ASSIGNMENTBULK_PROCESSCUSTALL '" +
					Request.QueryString["uid"] + "', '" +
					DDL_USERTO.SelectedValue.Trim() + "'";
				conn.ExecuteNonQuery();
