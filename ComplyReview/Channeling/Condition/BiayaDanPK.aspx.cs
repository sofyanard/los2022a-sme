...
namespace SME.ComplyReview.Channeling.Condition
{
	...
	public partial class BiayaDanPK : System.Web.UI.Page
	{
		...
		protected void btn_cari_Click(object sender, System.EventArgs e)
		{
			...
			conn.ExecuteQuery();
			year = conn.GetFieldValue("THEYEAR");
			...
			for(int i=0; i< dgListChan.Items.Count; i++)
			{
				...
				if(TXT_NO_PK != null)
				{
					conn.QueryString = "SELECT RFPRODUCT.PRODUCTDESC FROM RFPRODUCT, APPLICATION, CUSTPRODUCT " +
						"WHERE CUSTPRODUCT.AP_REGNO = APPLICATION.AP_REGNO  "+
						"AND CUSTPRODUCT.PRODUCTID = RFPRODUCT.PRODUCTID AND APPLICATION.AP_REGNO = '" +  dgListChan.Items[i].Cells[0].Text.ToString() + "'";
					conn.ExecuteQuery();
					
					TXT_NO_PK.Text = TXT_NO_PK.Text.ToString() + ((int)(num++)).ToString() + "/" + conn.GetFieldValue("PRODUCTDESC") + "/" + year;
				}
			}
		}
	}
