	...
	public partial class GeneralInfoCBI : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				LBL_AP_RELMNGR.Text = userid;
				TXT_AP_RELMNGR.Text = Session["FullName"].ToString();

				//pipeline
				temp_userid.Text = userid;
				...
				if (Request.QueryString["exist"] == "1")
				{
					...
					if(conn.GetRowCount() != 0) 
					{
						...
						try {DDL_AP_SIGNDATE_MONTH.SelectedValue = GlobalTools.FormatDate_Month(conn.GetFieldValue("PRE_APPENTRYDATE"));}
						catch {}
		...
		private void ViewData()
		{
			...
			try {DDL_lokasiproyek.SelectedValue=conn.GetFieldValue("CU_lokasiproyek");}
			catch {}
			...
			if (conn.GetFieldValue("CU_CUSTTYPEID") == "02")
			{
				...
				try {DDL_CU_IDCARDEXP_MONTH.SelectedValue = tool.FormatDate_Month(conn.GetFieldValue("CU_IDCARDEXP"));}
				catch {}
			...
			if (temp_grpunit.Text == "CO")
			{
				...
				try {DDL_AP_SIGNDATE_MONTH.SelectedValue=tool.FormatDate_Month(conn.GetFieldValue("ap_signdate"));}
				catch {}
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			if (RDO_RFCUSTOMERTYPE.SelectedValue == "01")	// badan usaha
			{
				...
				try 
				{
					conn.QueryString = "exec IDE_GENINFO_COMP_INSERT5 '" + 
			...
			else	
			{
				...
				try 
				{
					...
					conn.QueryString = "exec IDE_GENINFO_PERSON_INSERT5 '" + Request.QueryString["curef"] + "', " + 
					...
					conn.QueryString = "exec IDE_GENINFO_PERSON_INSERT6 '" + Request.QueryString["curef"] + "', " + 
