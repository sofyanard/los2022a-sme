...
namespace SME.CBI
{
	...
	public partial class CustomerInfoExport : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			conn.ExecuteQuery();
			LBL_USER_ID.Text = conn.GetFieldValue("CU_RM");
		...
		private void ViewFileExport()
		{
			...
			if (conn.GetRowCount() > 0) 
			{
				...
				conn.QueryString = "select * from CUST_EXPORT where CU_REF = '"+ Request.QueryString["curef"] + "' and FU_USERID = '" + LBL_USER_ID.Text.ToString() + "' and EXPORT_ID = " + tool.ConvertNull(DDL_FORMATFILE.SelectedValue);
				conn.ExecuteQuery();
		...
		private string Export_Word()
		{
			...
			fileNm = Request.QueryString["curef"] + "-" + nota + "-" + LBL_USER_ID.Text.ToString() + ".DOC";
			...
			{	
				...
				/// 
				conn.QueryString = "exec IN_CUST_EXPORT '" + nota +"','" + Request.QueryString["curef"] + "','" + fileNm + "', '" + LBL_USER_ID.Text.ToString() + "', '1'";
		...
		private string Export_Excel()
		{
			...
			fileNm = Request.QueryString["curef"] + "-" + nota + "-" + LBL_USER_ID.Text.ToString() + ".XLS";	// file hasil export
			fileOut = path + fileNm;		
		...
		private void ViewUploadFiles()
		{
			...
			for (int i = 1; i <= DatGrid.Items.Count; i++)
			{
				...
				if (LBL_USER_ID.Text.ToString().Trim() != DatGrid.Items[i-1].Cells[2].Text)
		...
		private void DATA_EXPORT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					...
					conn.QueryString = "exec IN_CUST_EXPORT '" + e.Item.Cells[0].Text +"','" + Request.QueryString["curef"] + "', '','" + LBL_USER_ID.Text.ToString() + "', '2'";
					conn.ExecuteQuery();
			...
			if (conn.GetRowCount() > 0)
			{
				...
				for (int i = 0; i < uploadedFiles.Count; i++)
				{
					...
					try
					{
						...
						{
							outputfilename = Request.QueryString["curef"].Trim() + "-"+ LBL_USER_ID.Text.ToString() +"-" + Path.GetFileName(userPostedFile.FileName);
							...
							conn.QueryString = "EXEC CUSTINFO_UPLOADSAVE '1', '" + Request.QueryString["curef"] +
								"', '', '" + LBL_USER_ID.Text.ToString().Trim() + "', '" + outputfilename + "'";
							conn.ExecuteQuery();
