...
{
	/// <summary>
	/// Summary description for RptSLACRMApprovalPrint.
	/// </summary>
	public partial class RptSLACRMApprovalPrint : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				loadData(tanggal1, tanggal2,region, cbc, branch, teamleader, rm, approval);
			}
		}
		private void loadData(string tanggal1, string tanggal2,string region, string cbc, string branch, string teamleader, string rm, string approval)
		{
			string regionname="", cbcname="", branchname="", haridisp="", approvalname =""; 
			...
			if (!approval.Equals(""))
			{
				conn.QueryString= "select su_fullname from scuser where userid='" + approval + "' ";
				conn.ExecuteQuery();
				if (conn.GetRowCount()>0) 
				{
					approvalname =conn.GetFieldValue(0,"su_fullname").ToUpper();
				}
				else
				{
					approvalname ="ALL";
            ...
            LBL_APPROVAL.Text=approvalname.ToUpper();

			conn.QueryString = "SELECT * FROM TMP_REPORT_CRMAPPROVAL WHERE userid = '" + Session["UserID"].ToString() + "' order by HARI asc";
			conn.ExecuteQuery();
