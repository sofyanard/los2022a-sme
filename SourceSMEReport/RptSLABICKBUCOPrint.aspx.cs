...
{
	/// <summary>
	/// Summary description for RptSLABICKBUCOPrint.
	/// </summary>
	public partial class RptSLABICKBUCOPrint : System.Web.UI.Page
	{
		...
		{
			string regionname="", cbcname="", branchname="", haridisp = ""; //rmname="", 
			...
			conn.QueryString = "SELECT * FROM TMP_REPORT_SLABICKBUCO WHERE userid = '" + Session["UserID"].ToString() + "' order by [jumlah hari] asc";
			conn.ExecuteQuery();
			for (int i = 0; i < conn.GetRowCount(); i++)
			{
				switch(conn.GetFieldValue(i, "jumlah hari"))
				{
					case "1" : 
						haridisp = "=1";
						break;
				...
				TBL_CONTENT.Rows[i + 1].Cells[0].Text = "&nbsp;" + haridisp.ToString(); 
				TBL_CONTENT.Rows[i + 1].Cells[0].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[0].CssClass= "ItemPrint";
				
				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[1].Text = "&nbsp;" + conn.GetFieldValue(i, "Jumlah Applikasi").Trim();
