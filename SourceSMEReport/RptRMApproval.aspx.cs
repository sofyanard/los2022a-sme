...
{
	/// <summary>
	/// Summary description for RptRMApproval.
	/// </summary>
	public partial class RptRMApproval : System.Web.UI.Page
	{
		...
		private void Load_ReportViewer(string action,string sql_kondisi, string Start_Date, string End_Date, string region, string CBC, string Branch, string teamleader, string approval)
		{
			...
			*/
			...
			string vAccept="", vReject="", vCompleted="", vPending="";
			Conn.QueryString = "select count(*) jml from RPT_RMAPPROVED where ad_status='Accept' " + sql_kondisi;
			Conn.ExecuteQuery();
			if (Conn.GetRowCount()>0)
			{vAccept=Conn.GetFieldValue(0,"jml");}
			else
			{vAccept="0";}
			
			Conn.QueryString = "select count(*) jml from RPT_RMAPPROVED where ad_status='Reject' " + sql_kondisi;
			Conn.ExecuteQuery();
			if (Conn.GetRowCount()>0)
			{vReject=Conn.GetFieldValue(0,"jml");}
			else
			{vReject="0";}

			Conn.QueryString = "select count(*) jml from RPT_RMAPPROVED where RmApprovedDate<>'' and RmApprovedDate is not null "  + sql_kondisi;
			Conn.ExecuteQuery();
			if (Conn.GetRowCount()>0)
			{vCompleted=Conn.GetFieldValue(0,"jml");}
			else
			{vCompleted="0";}

			Conn.QueryString = "select count(*) jml from RPT_RMAPPROVED where (RmApprovedDate='' or RmApprovedDate is null) " + sql_kondisi; 
			Conn.ExecuteQuery();
			if (Conn.GetRowCount()>0)
			{vPending=Conn.GetFieldValue(0,"jml");}
			else
			{vPending="0";}

            if (!action.Equals("PRINT"))
            {
                //ReportViewer1.ReportPath = "/SMEReports/RptCRMApproval&sql_kondisi=" + sql_kondisi + "&Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&region=" + region + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&approval=" + approval + "&Accept=" + vAccept + "&Reject=" + vReject + "&Completed=" + vCompleted + "&Pending=" + vPending + "&rs:Command=Render&rc:Toolbar=True";
                ...
                ReportViewer1.ServerReport.ReportPath = "/SMEReports/RptCRMApproval";
                ...
                paramList.Add(new ReportParameter("Accept", vAccept, false));
                paramList.Add(new ReportParameter("Reject", vReject, false));
                paramList.Add(new ReportParameter("Completed", vCompleted, false));
            ...
            {
                sql_kondisi = Server.HtmlEncode(sql_kondisi.Replace("'", "''"));
                Response.Redirect("RptCRMApprovalPrint.aspx?sql_kondisi=" + sql_kondisi + "&Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&region=" + region + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&approval=" + approval + "&Accept=" + vAccept + "&Reject=" + vReject + "&Completed=" + vCompleted + "&Pending=" + vPending + " ");
            }
		}
			...
			*/
			{
				...
				if (!approval.Equals(""))
				{
					kriterianya+="and (rmapprover = '" + approval + "') ";
				}
				
				Load_ReportViewer(action,kriterianya, tanggal1, tanggal2,region, cbc,branch, teamleader, approval);
			}
		}
