...
{
	/// <summary>
	/// Summary description for RptPSPerformancePrint.
	/// </summary>
	public partial class RptPSPerformancePrint : System.Web.UI.Page
	{
		...
		private void Load_Data(string sql_kondisi, string Start_Date, string End_Date, string region, string cbc, string branch, string PS, string teamleader)
		{
			...
			Conn.QueryString = "exec Rpt_PSPerformance '" + sql_kondisi + "' ";
			Conn.ExecuteQuery();
			for (int i = 0; i < Conn.GetRowCount(); i++)
			{
				...
				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[4].Text = "&nbsp;" + tools.FormatDate(Conn.GetFieldValue(0,"Start_Date"),false);
				TBL_CONTENT.Rows[i + 1].Cells[4].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[4].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[5].Text = "&nbsp;" + tools.FormatDate(Conn.GetFieldValue(0,"End_Date"), false);
				TBL_CONTENT.Rows[i + 1].Cells[5].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[5].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[6].Text = "&nbsp;" + Conn.GetFieldValue(0,"Jml_Hari");
				TBL_CONTENT.Rows[i + 1].Cells[6].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[6].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[7].Text = "&nbsp;" + Conn.GetFieldValue(0,"su_fullname");
				TBL_CONTENT.Rows[i + 1].Cells[7].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[7].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[8].Text = "&nbsp;" + Conn.GetFieldValue(0,"BU_Comment1");
				TBL_CONTENT.Rows[i + 1].Cells[8].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[8].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[9].Text = "&nbsp;" + Conn.GetFieldValue(0,"PK_Analis");
				TBL_CONTENT.Rows[i + 1].Cells[9].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[9].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[10].Text = "&nbsp;" + Conn.GetFieldValue(0,"Account_Opinion");
				TBL_CONTENT.Rows[i + 1].Cells[10].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[10].CssClass= "ItemPrint_d";

				TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + 1].Cells[11].Text = "&nbsp;" + Conn.GetFieldValue(0,"RM");
				TBL_CONTENT.Rows[i + 1].Cells[11].HorizontalAlign = HorizontalAlign.Center;
				TBL_CONTENT.Rows[i + 1].Cells[11].CssClass= "ItemPrint_d";

			}

			string vCompleted = "0", vPending="0";
			//Conn.QueryString = "select count(*) jml from vw_report_prrkperformance where (End_Date is not null and End_Date<>'') " + sql_kondisi + " ";
			Conn.QueryString = "exec Rpt_psPerformanceFooter '" + " and (End_Date is not null and End_Date<>'''') " + sql_kondisi + "' ";
			Conn.ExecuteQuery();
			if (Conn.GetRowCount()>0)
			{
				vCompleted = Conn.GetFieldValue(0,"jml");
			}
			LBL_COMPLETED.Text = vCompleted.ToString();

			//Conn.QueryString= "select count(*) jml from vw_report_prrkperformance where (End_Date is null or End_Date='') " + sql_kondisi + " ";
			Conn.QueryString = "exec Rpt_psPerformanceFooter '" + " and (End_Date is null or End_Date='''') " + sql_kondisi + "' ";
			Conn.ExecuteQuery();
			if (Conn.GetRowCount()>0)
			{
				vPending = Conn.GetFieldValue(0,"jml");
			}
			LBL_PENDING.Text = vPending.ToString();
		}
