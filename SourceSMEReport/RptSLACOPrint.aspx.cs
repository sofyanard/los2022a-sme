...
{
	/// <summary>
	/// Summary description for RptSLACOPrint.
	/// </summary>
	public partial class RptSLACOPrint : System.Web.UI.Page
	{
		...
		private void loadData(string sql_kondisi, string Start_Date, string End_Date,string region, string cbc, string branch, string teamleader)
		{
			...
			Conn.QueryString = "exec Rpt_SLA_CO '" + sql_kondisi + "' ";
			Conn.ExecuteQuery(1800);
			for (int i = 0; i < Conn.GetRowCount(); i++)
			{
				switch(Conn.GetFieldValue(i, "urut"))
				{
					case "1" : 
						haridisp = "=1";
						break;
