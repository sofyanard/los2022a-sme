...
{
	/// <summary>
	/// Summary description for RptSLACollApprPrint.
	/// </summary>
	public partial class RptSLACollApprPrint : System.Web.UI.Page
	{
		...
		private void loadData(string tanggal1, string tanggal2,string region, string cbc, string branch, string teamleader, string rm)
		{
			...
			conn.QueryString = "SELECT * FROM TMP_REPORT_COLLAPPR WHERE userid = '" + Session["UserID"].ToString() + "' order by [JM HARI] asc";
			conn.ExecuteQuery();
