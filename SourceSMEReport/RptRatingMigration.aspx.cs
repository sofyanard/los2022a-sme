...
{
	/// <summary>
	/// Summary description for RptRatingMigration.
	/// </summary>
	public partial class RptRatingMigration : System.Web.UI.Page
	{
		...
		private void Load_ReportViewer(string ddl_tahun)
		{
			...
			ReportViewer1.ReportPath = "/SMEReports/RptRatingMigration&sql_kondisi=" + Server.HtmlEncode(sql_kondisi) + "&tahun=" + ddl_tahun + "&rs:Command=Render&rc:Toolbar=True";
		}
		...
		private void Load_ReportPrint(string ddl_tahun)
		{
			...
			start_year = double.Parse(ddl_tahun.ToString())-3;
			sql_kondisi += " and year between " + start_year.ToString() + " and " + end_year.ToString(); 
			Response.Redirect("RptRatingMigrationPrint.aspx?sql_kondisi=" + Server.HtmlEncode(sql_kondisi) + "&start_year=" + start_year + "&end_year=" + end_year);
