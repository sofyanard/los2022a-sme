...
namespace SourceSMEReport
{
	...
	public partial class WebForm1 : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            /*
			Conn.QueryString = "Delete from TMP_TBLBUSINESSUNIT where userid='" + Session["UserID"].ToString()  + "' ";
			Conn.ExecuteQuery();
            */
		...
		private void Load_Data()
		{
			...
			*/
			Load_ReportViewer(Session["UserID"].ToString(), region, cbc, branch, rm, tanggal1, tanggal2, teamleader);
			
		}

		private void Load_ReportViewer(string userid, string region, string cbc, string branch, string rm, string Start_Date, string End_Date, string teamleader)
		{
            ...
            /*
            ReportViewer2.ServerUrl = "http://" + ReportAddr + "/" + ReportDir;
			ReportViewer2.ReportPath = "/SMEReports/SLABussinesUnit&userid=" + userid + "&REGION=" + region + "&CBC=" + cbc + "&Branch=" + branch + "&RM=" + rm + "&Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&teamleader=" + teamleader + "&rs:Command=Render&rc:Toolbar=True";
            */
            ...
            ReportViewer2.ServerReport.ReportPath = "/SMEReports/SLABussinesUnit";
			...
			if (Tools.isDateValid(this,TXT_TGL1.Text,DDL_BLN1.SelectedValue,TXT_THN1.Text)&&Tools.isDateValid(this, TXT_TGL2.Text,DDL_BLN2.SelectedValue,TXT_THN2.Text))
			{
				...
				*/
				...
				{ 
					...
					kriterianya = "WHERE (AP_RelMngr = ''" + rm + "'') AND (convert(varchar,BS_REQDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") "+
						"AND ((convert(varchar,BS_RECVDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") or BS_RECVDATE is null or BS_RECVDATE='''') ";
					if (!teamleader.Equals(""))
					{
				...
				else
				{
					if(!branch.Equals(""))
					{
						kriterianya = "WHERE (BRANCH_CODE = ''" + branch + "'') AND ((convert(varchar,BS_REQDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") or BS_REQDATE=''" + tanggal2_k + "'') "+
							"AND ((convert(varchar,BS_RECVDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") or BS_RECVDATE is null or BS_RECVDATE='''') ";
						if (!teamleader.Equals(""))
						{
							kriterianya += "and su_teamleader= ''" + teamleader + "'' " ;
					...
					else
					{
						if (!cbc.Equals(""))
						{
							kriterianya = "WHERE (CBC_CODE = ''" + cbc + "'' ) AND (convert(varchar,BS_REQDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") "+
								"AND ((convert(varchar,BS_RECVDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") or BS_RECVDATE is null or BS_RECVDATE='''') ";
							if (!teamleader.Equals(""))
							{
								kriterianya += "and su_teamleader= ''" + teamleader + "'' " ;
						...
						else
						{
							if(!region.Equals(""))
							{
								kriterianya = "WHERE (areaid= ''" + region + "'') AND (convert(varchar,BS_REQDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ")"+
									"AND ((convert(varchar,BS_RECVDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") or BS_RECVDATE is null or BS_RECVDATE='''') ";
								if (!teamleader.Equals(""))
								{
									kriterianya += "and su_teamleader= ''" + teamleader + "'' " ;
							...
							{
								kriterianya = "WHERE (convert(varchar,BS_REQDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ")"+
									"AND ((convert(varchar,BS_RECVDATE,112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") or BS_RECVDATE is null or BS_RECVDATE='''') ";
								if (!teamleader.Equals(""))
								{
									kriterianya += "and su_teamleader= ''" + teamleader + "'' " ;
                ...
                */
				...
				Conn.QueryString = " exec Rpt_BusinessUnitBIReport '" + Session["UserID"].ToString() + "','" + kriterianya + "'";
				Conn.ExecuteQuery(1800); 
				if (!action.Equals("PRINT")) 
					Load_ReportViewer(Session["UserID"].ToString(), region, cbc, branch, rm, tanggal1, tanggal2, teamleader);
				else
					Response.Redirect("RptBusinessUnitPrint.aspx?userid=" + Session["UserID"].ToString() + "&tanggal1=" + tanggal1 + "&tanggal2=" + tanggal2 + "&region=" + region + "&cbc=" + cbc + "&branch=" + branch + "&teamleader=" + teamleader + "&rm=" + rm);
			}
			else
			{
