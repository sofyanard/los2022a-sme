...
{
	/// <summary>
	/// Summary description for RptSLABussinessUnitPrint.
	/// </summary>
	public partial class RptSLABussinessUnitPrint : System.Web.UI.Page
	{
		...
		private void loadData(string tanggal1, string tanggal2,string region, string cbc, string branch, string teamleader, string rm)
		{
		    ...
		    conn.QueryString = "SELECT * FROM TMP_REPORT_SLABUSSUNIT WHERE userid = '" + Session["UserID"].ToString() + "' order by jml_hari asc";
			conn.ExecuteQuery();
			for (int i = 0; i < conn.GetRowCount(); i++)
			{
				switch(conn.GetFieldValue(i, "JML_HARI"))
					{
					case "1" : 
					    haridisp = "=1";
						break;
