...
{
	/// <summary>
	/// Summary description for RptSLACRMPrint.
	/// </summary>
	public partial class RptSLACRMPrint : System.Web.UI.Page
	{
		...
		private void loadData(string tanggal1, string tanggal2,string region, string cbc, string branch, string teamleader, string rm, string approval)
		{
			...
			if(!cbc.Equals(""))
			{
				this.LBL_CBC.Text = cbcname.ToUpper();
			}
			else
			{
			...
			conn.QueryString = "select hari, [jumlah applikasi], jumlah, userid  from TMP_REPORT_CRM where userid='" + Session["UserID"].ToString() + "' order by hari asc";
			conn.ExecuteQuery();
