...
{
	/// <summary>
	/// Summary description for RptCollAppraisal.
	/// </summary>
	public partial class RptCollAppraisal : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				Conn.QueryString = "Delete from TMP_REPORT_COLLAPPR where userid='" + Session["UserID"].ToString() + "' ";
				Conn.ExecuteQuery();
			}
		}
		...
		private void Load_ReportViewer(string Start_Date, string End_Date, string region, string CBC, string Branch, string teamleader)
		{
			...
			*/
            ...
            //ReportViewer1.ReportPath = "/SMEReports/SLACollAppraisal&userid=" + Session["UserID"].ToString() + "&START_DATE=" + tanggal1 + "&END_DATE=" + tanggal2 + "&area=" + region + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&rs:Command=Render&rc:Toolbar=True";
            ...
            ReportViewer1.ServerReport.ReportPath = "/SMEReports/SLACollAppraisal";
		...
		private void LoadSql(string action)
		{
			...
			*/
			{
				//(BRANCH_CODE = @BRANCH) AND (CBC_CODE = @CBC)AND (areaid = @area)
				Conn.QueryString ="Delete from TMP_REPORT_COLLAPPR where userid='" + Session["UserID"].ToString() + "'";
				Conn.ExecuteQuery();
				...
				Conn.QueryString = "insert into TMP_REPORT_COLLAPPR "+
					"SELECT [JM HARI], COUNT(*) AS Expr1, ISNULL(SUM(GrossSales), 0) AS Jumlah, '" + Session["UserID"].ToString() + "' as userid "+
					"FROM VW_REPORT_COLLAPPR2 "+
					"WHERE (convert(varchar, LA_ASSIGNDATE, 112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k + ") AND (convert(varchar, LA_COMPLETEDATE, 112) > " + tanggal1_k +") " + kriterianya + " "+
					"GROUP BY [JM HARI] ";
				Conn.ExecuteQuery(1800);
				if (!action.Equals("PRINT"))
					Load_ReportViewer(tanggal1,tanggal2,region, cbc,branch, teamleader);
				else
					Response.Redirect("RptSLACollApprPrint.aspx?userid=" + Session["UserID"].ToString() + "&tanggal1=" + tanggal1 + "&tanggal2=" + tanggal2 + "&region=" + region + "&cbc=" + cbc + "&branch=" + branch + "&teamleader= " +teamleader);
			}
		}
