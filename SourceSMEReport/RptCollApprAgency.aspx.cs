...
{
	/// <summary>
	/// Summary description for RptCollApprAgency.
	/// </summary>
	public partial class RptCollApprAgency : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				for (int i = 0; i < Conn.GetRowCount(); i++)
				{
					...
					DDL_AGENCY.Items.Add(li);
				}
				
				Conn.QueryString = "Delete from TMP_REPORT_COLLAPPRAGENCY where userid='" + Session["UserID"].ToString() + "'";
				Conn.ExecuteQuery();
		...
		private void Load_ReportViewer(string Start_Date, string End_Date, string region, string CBC, string Branch, string teamleader, string agency)
		{
			/*
			...
			*/
            ...
            //ReportViewer1.ReportPath = "/SMEReports/SLACollAppraisalAgency&userid=" + Session["UserID"].ToString() + "&START_DATE=" + tanggal1 + "&END_DATE=" + tanggal2 + "&AREA=" + region + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&agency=" + agency + "&rs:Command=Render&rc:Toolbar=True";			
			...
			*/
			{
				...
				Conn.QueryString = "insert into TMP_REPORT_COLLAPPRAGENCY "+
					"SELECT [JM HARI], COUNT(*) AS Expr1, ISNULL(SUM(GrossSales), 0) AS Jumlah, '" + Session["UserID"].ToString()  + "' "+
					"FROM VW_REPORT_COLLAPPRAGENCY2 "+
					"WHERE (convert(varchar, LA_ASSIGNDATE, 112) BETWEEN " + tanggal1_k + " AND " + tanggal2_k +") " + kriterianya + " "+
					"GROUP BY [JM HARI] ";
				Conn.ExecuteQuery(1800);
				if (!action.Equals("PRINT"))
					Load_ReportViewer(tanggal1,tanggal2,region, cbc,branch, teamleader, agency);
				else
					Response.Redirect("RptCollAppraisalAgencyPrint.aspx?userid=" + Session["UserID"].ToString() + "&tanggal1=" + tanggal1 + "&tanggal2=" + tanggal2 + "&region=" + region + "&cbc=" + cbc + "&branch=" + branch + "&agency=" + agency + "&teamleader=" + teamleader);
			}
		}
