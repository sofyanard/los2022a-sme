...
{
	/// <summary>
	/// Summary description for RptILPUtilizationRenewal.
	/// </summary>
	public partial class RptILPUtilizationRenewal : System.Web.UI.Page
	{
		...
		private void LoadSql(string action)
		{
			...
			if (Tools.isDateValid(this,TXT_TGL1.Text,DDL_BLN1.SelectedValue,TXT_THN1.Text)&&Tools.isDateValid(this, TXT_TGL2.Text,DDL_BLN2.SelectedValue,TXT_THN2.Text))
			{
				...
				if (!Information.IsDate(tanggal1) || !Information.IsDate(tanggal2))
				{
					...
					LoadReport_Load(tanggal1, tanggal2, businessunit, area, branch);
			}
			else
			{
		...
		private void LoadReport_Load(string tanggal1, string tanggal2, string businessunit, string area, string branch)
		{
			string ReportAddr="";
			...
			ReportViewer1.ReportPath = "/SMEReports/RptDashboardRenewal&date1="+ tanggal1 + "&date2=" + tanggal2 + "&businessunit=" + businessunit +  "&area=" + area + "&branch=" + branch + "&rs:Command=Render";
		}
