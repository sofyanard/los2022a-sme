...
namespace SME.SourceSMEReport.ILPDashBoard
{
	...
	public partial class RptOverallSLA : System.Web.UI.Page
	{
		...
		private void fillProcessType()
		{
			...
			Conn.QueryString = "select code, flow_name from rf_tat order by code ";
			Conn.ExecuteQuery();
		...
		private void LoadSql(string action)
		{
			...
			if (Tools.isDateValid(this,TXT_TGL1.Text,DDL_BLN1.SelectedValue,TXT_THN1.Text)&&Tools.isDateValid(this, TXT_TGL2.Text,DDL_BLN2.SelectedValue,TXT_THN2.Text))
			{
				...
				if (!Information.IsDate(tanggal1) || !Information.IsDate(tanggal2))
				{
					...
					LoadReport_Load(tanggal1, tanggal2, businessunit, area, branch, apptype, protype);
			}
			else
			{
		...
		private void LoadReport_Load(string tanggal1, string tanggal2, string businessunit, string area, string branch, string apptype, string protype)
		{
			string ReportAddr="";
			...
			ReportViewer1.ReportPath = "/SMEReports/RptDashboardTATEndToEndProcess&date1="+ tanggal1 + "&date2=" + tanggal2 + "&businessunit=" + businessunit +  "&area=" + area + "&branch=" + branch + "&apptype=" + apptype + "&protype=" + protype + "&rs:Command=Render";
		}
