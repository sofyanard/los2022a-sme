...
{
	/// <summary>
	/// Summary description for RptDisbursementBranch.
	/// </summary>
	public partial class RptDisbursementBranch : System.Web.UI.Page
	{
		...
		private void fillBranch()
		{
			...
			{
				case "1": 
					Conn.QueryString = "select distinct branch_code, branch_name "+
						"from rfbranch b left join rfcity c on b.cityid=c.cityid where cbc_Code is not null and branch_code <>'' and b.areaid = '" + DDL_REGION.SelectedValue + "' "+
				...
				default:
					Conn.QueryString = "select distinct branch_code, branch_name "+
						"from rfbranch b left join rfcity c on b.cityid=c.cityid where branch_Code is not null and branch_code <>'' and b.areaid = '" + DDL_REGION.SelectedValue + "' ";
		...
		{
			Conn.QueryString = "SELECT BUSSTYPEID, BUSSTYPEDESC FROM RFBUSINESSTYPE WHERE ACTIVE = '1' AND LEN(BUSSTYPEID) < 3 order by busstypeid ";
			Conn.ExecuteQuery();
		...
		{
			string sql_kondisi="",tanggal1="", tanggal1_k="", tanggal2="", tanggal2_k="", region, branch="", industri="", program="", product="";
			...
			if (!branch.Equals(""))
			{
				sql_kondisi +=" and branch_code='" + branch + "' ";
			}
			if (!industri.Equals(""))
			{
			...
			*/
			...
			if (!action.Equals("PRINT"))
				Load_ReportViewer(sql_kondisi, tanggal1,tanggal2,region, branch, industri, program, product);
			else
				Response.Redirect("RptDisbursementBranchPrint.aspx?sql_kondisi=" + sql_kondisi.Replace("'","''") + "&Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&region=" + region + "&branch=" + branch + "&industri=" + industri + "&program=" + program + "&product=" + product);
		}

		private void Load_ReportViewer(string sql_kondisi, string Start_Date, string End_Date, string region, string Branch, string industri, string program, string product)
		{
			/*
			...
			*/
            ...
            //ReportViewer1.ReportPath = "/SMEReports/RptDisburse&sql_kondisi=" + Server.HtmlEncode(sql_kondisi) + "&Start_Date=" + Start_Date + "&End_Date=" + End_Date + "&region=" + region + "&BRANCH=" + Branch + "&INDUSTRI=" + industri + "&program=" + program + "&PRODUCT=" + product + "&rs:Command=Render&rc:Toolbar=True";				
            ...
            ReportViewer1.ServerReport.ReportPath = "/SMEReports/RptDisburse";
