...
{
	/// <summary>
	/// Summary description for Agunan.
	/// </summary>
	public partial class Agunan : System.Web.UI.Page
	{
		...
		private void fillWilayah()
		{
			...
			DDL_WILAYAH.Items.Clear();
			DDL_WILAYAH.Items.Add(new ListItem("-- PILIH --",""));
			conn2.QueryString = "select WILAYAH, KANWIL from PAR_WILAYAH";
			conn2.ExecuteQuery();
			for (int i = 0; i < conn2.GetRowCount(); i++)
			{
				DDL_WILAYAH.Items.Add(new ListItem(conn2.GetFieldValue(i,0) + " - " + conn2.GetFieldValue(i,1),conn2.GetFieldValue(i,0)));
			}	*/
		}
		...
		private void fillKelompok()
		{
			...
			/*DDL_KELOMPOK.Items.Clear();
			DDL_KELOMPOK.Items.Add(new ListItem("-- PILIH --",""));
			DDL_KELOMPOK.Items.Add(new ListItem("Corporate Banking","CORP"));
			DDL_KELOMPOK.Items.Add(new ListItem("Commercial Banking","COMM"));
			DDL_KELOMPOK.Items.Add(new ListItem("Business Banking","BB"));
			DDL_KELOMPOK.Items.Add(new ListItem("Micro Banking","MICRO"));
			DDL_KELOMPOK.Items.Add(new ListItem("Consumer Loan","CL"));
			DDL_KELOMPOK.Items.Add(new ListItem("Credit Recovery","CR"));*/
		}
...
*/
		...
		private void fillUnit() 
		{
			...
			if (DDL_KELOMPOK.SelectedValue=="")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc";
			}
			else if (DDL_KELOMPOK.SelectedValue=="CORP")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc where (left(BUC,2)='CB' or left(BUC,2)='IB') and left(BUC,3)<>'CBC'";
			}
			else if (DDL_KELOMPOK.SelectedValue=="COMM")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc where left(BUC,3)='CBC'";
			}
			else if (DDL_KELOMPOK.SelectedValue=="BB")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc where left(BUC,2)='SM'";
			}
			else if (DDL_KELOMPOK.SelectedValue=="MICRO")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc where left(BUC,2)='MB'";
			}
			else if (DDL_KELOMPOK.SelectedValue=="CL")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc where left(BUC,2)='CL'";
			}
			else if (DDL_KELOMPOK.SelectedValue=="CR")
			{
				conn2.QueryString = "select buc, buc_desc from par_buc where left(BUC,2)='CR'";
			}
			conn2.ExecuteQuery();
			for (int i = 0; i < conn2.GetRowCount(); i++)
			{
				DDL_UNIT.Items.Add(new ListItem(conn2.GetFieldValue(i,0) + " - " + conn2.GetFieldValue(i,1),conn2.GetFieldValue(i,0)));
			}	
			*/
		}
		...
		private void LoadReport_Load(string wilayah, string unit, string kelompok, string rco, string status)
		{
			...
			ReportViewer1.ReportPath = "/SMEReports/DQAControllingAgunan&wilayah="+ wilayah + "&unit=" + unit +  "&kelompok=" + kelompok +  "&rco=" + rco + "&status=" + status + "&rs:Command=Render";
		}
