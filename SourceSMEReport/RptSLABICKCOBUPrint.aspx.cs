...
{
	/// <summary>
	/// Summary description for RptBICKCOBUPrint.
	/// </summary>
	public partial class RptBICKCOBUPrint : System.Web.UI.Page
	{
		...
		private void loadData(string tanggal1, string tanggal2,string region, string cbc, string branch, string teamleader, string rm)
		{
			...
			conn.QueryString = "SELECT * FROM TMP_REPORT_SLABICKCOBU WHERE userid = '" + Session["UserID"].ToString() + "' order by [jumlah hari] asc";
				conn.ExecuteQuery();
				for (int i = 0; i < conn.GetRowCount(); i++)
				{
					...
					TBL_CONTENT.Rows[i + 1].Cells.Add(new TableCell());
					TBL_CONTENT.Rows[i + 1].Cells[2].Text = "&nbsp;" + tools.ConvertCurr(conn.GetFieldValue(i, "total jumlah"));
