...
{
	/// <summary>
	/// Summary description for RptRmPerformance.
	/// </summary>
	public partial class RptRmPerformance : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                ...
                /*
				Conn.QueryString = "Delete from TMP_REPORT_RMPERFORMANCE where userid='" + Session["UserID"].ToString() + "' ";
				Conn.ExecuteQuery();
                */
		...
		{
			string kriterianya = "", accept_code="", reject_code="";
			/*
			...
			*/
			...
			Conn.QueryString="insert into TMP_REPORT_RMPERFORMANCE "+
                             "SELECT SU_FULLNAME, Branch_Name, ProductDesc, Industri, apptypedesc, ISNULL(COUNT(*), 0) AS jm_app, "+
                             "ISNULL(SUM(Jumlah), 0) AS jumlah, ISNULL(SUM(Disbursed), 0)  AS disbursed, PROG_CODE, programdesc, cp_decsta, '" + Session["UserID"].ToString() + "' AS userid, ap_relmngr, su_teamleader FROM "+
                             "VW_REPORT_RMPERFORMANCE "+
                             "WHERE (convert(varchar, ap_signdate, 112) BETWEEN '" + tanggal1_k + "' AND '" + tanggal2_k + "') "+
                             "AND (PROG_CODE = '" + program + "') " + kriterianya + " "+
                             "GROUP BY ap_relmngr, SU_FULLNAME, Branch_Name, ProductDesc, Industri, apptypedesc, PROG_CODE, cp_decsta, su_teamleader, programdesc ";
			Conn.ExecuteQuery(1800);

            if (!action.Equals("PRINT"))
            {
                //ReportViewer1.ReportPath = "/SMEReports/RptRMPerformance&userid=" + Session["UserID"].ToString() + "&Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&PROGRAM=" + program + "&PRODUCT=" + product + "&INDUSTRI=" + industri + "&accept_code=" + accept_code + "&reject_code=" + reject_code + "&rs:Command=Render&rc:Toolbar=True";\
                ...
                ReportViewer1.ServerReport.ReportPath = "/SMEReports/SLABussinesUnit";
                ...
                Response.Redirect("RptRmPerformancePrint.aspx?Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&PROGRAM=" + program + "&PRODUCT=" + product + "&INDUSTRI=" + industri);
		...
		{
				Response.Redirect("MainReportPR.aspx?mc=" + Request.QueryString["mc"]);
		}
