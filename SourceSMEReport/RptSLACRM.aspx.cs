...
{
	/// <summary>
	/// Summary description for RptSLACRM.
	/// </summary>
	public partial class RptSLACRM : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				LBL_REGION.Text = Posisi_User().ToString();
				fillRegion();
			}
			Conn.QueryString = "Delete from TMP_REPORT_SLABICKBUCO where userid='" + Session["UserID"].ToString() + "'";
			Conn.ExecuteQuery();
		}
		...
		private void Load_ReportViewer(string Start_Date, string End_Date, string region, string CBC, string Branch, string teamleader)
		{
			...
			*/
            ...
            //ReportViewer1.ReportPath = "/SMEReports/RptSLACRM&userid=" + Session["UserID"].ToString() + "&Start_Date=" + tanggal1 + "&End_Date=" + tanggal2 + "&region=" + region + "&CBC=" + CBC + "&BRANCH=" + Branch + "&teamleader=" + teamleader + "&rs:Command=Render&rc:Toolbar=True";
            ...
            ReportViewer1.ServerReport.ReportPath = "/SMEReports/RptSLACRM";
		...
		private void LoadSql(string action)
		{
			...
			*/
			{
				//AND  (BRANCH_CODE = @BRANCH) AND (CBC_CODE = @CBC) AND (areaid = @region)
				if(!branch.Equals(""))
				{
				...
				*/				
				Conn.QueryString = "Delete from TMP_REPORT_CRM where userid='" + Session["UserID"].ToString() + "'";
				Conn.ExecuteQuery();

				/*
								Conn.QueryString = "insert into TMP_REPORT_CRM "+
												   "SELECT [hari], COUNT(*) AS [Jumlah Applikasi], SUM([total amount]) AS Jumlah, '" + Session["UserID"].ToString() + "' AS Userid "+
												   "FROM VW_REPORT_CRM3 "+
								...
								Conn.ExecuteQuery();
				*/
				Conn.QueryString="exec sp_rpt_CRM '" + Session["UserID"].ToString() + "'," + tanggal1 + "," + tanggal2 + ",'" + kriterianya + "'";
				Conn.ExecuteQuery(1800);
				if (!action.Equals("PRINT"))
					Load_ReportViewer(tanggal1,tanggal2,region, cbc,branch, teamleader);
				else
					Response.Redirect("RptSLACRMPrint.aspx?userid=" + Session["UserID"].ToString() + "&tanggal1=" + tanggal1 + "&tanggal2=" + tanggal2 + "&region=" + region + "&cbc=" + cbc + "&branch=" + branch + "&teamleader=" + teamleader);
			}
		}
