...
{
	/// <summary>
	/// Summary description for RptDisbursementProdPrint.
	/// </summary>
	public partial class RptDisbursementProdPrint : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			Load_Data(sql_kondisi, Start_Date, End_Date, region, industri, program, product);
		}

		private void Load_Data(string sql_kondisi, string Start_Date, string End_Date, string region, string industri, string program, string product)
		{
			string regionname=""; 
			...
			Conn.QueryString = "exec Rpt_DisbursementProdPrint '" + sql_kondisi +  "'";
			Conn.ExecuteQuery();
			string pre_area="";
			int k=0;
			for (int i = 0; i < Conn.GetRowCount(); i++)
			{
				...
				TBL_CONTENT.Rows[i + k + 1].Cells.Add(new TableCell());
				TBL_CONTENT.Rows[i + k + 1].Cells[1].Text = "&nbsp;" + Conn.GetFieldValue(i, "productdesc").ToString();
