...
namespace SME.LOW
{
	...
	/// </summary>
	public partial class LOWSearchCustomer : System.Web.UI.Page
	{
...
//				if (conn.GetRowCount() != 0)
			...
			else if (stop == false && (Textbox2.Text != "" && Textbox1.Text != ""))
			{
				conn.QueryString = "select cu_ref, cu_cif, cu_firstname + ' ' + cu_middlename + ' ' + cu_lastname as cust_name, cu_npwp from vw_cust_personal where CU_DOB = CONVERT(DATETIME, '" + Textbox1.Text.ToString() + "-" + DDL_CU_DOB_MM.SelectedValue.ToString() + "-" + Textbox2.Text.ToString() + "')"+
					" union "+
					"select cu_ref, cu_cif, cu_compname as cust_name, cu_npwp from vw_cust_company where CU_COMPESTABLISH = CONVERT(DATETIME, '" + Textbox1.Text.ToString() + "-" + DDL_CU_DOB_MM.SelectedValue.ToString() + "-" + Textbox2.Text.ToString() + "')";
				conn.ExecuteQuery();
				//				if (conn.GetRowCount() != 0)
				FillGrid();				
			}
		...
		private void DatGrd_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "View":
					...
					{
						conn.QueryString = "SELECT TOP 1 TRACKCODE FROM RFTRACK WHERE TRACKCODE = 'LOW0.0'";
						conn.ExecuteQuery();

						string tc = conn.GetFieldValue(0,0);
						...
						Response.Redirect("LOWKetentuanKredit.aspx?curef=" + e.Item.Cells[0].Text + "&mc=" + Request.QueryString["mc"] + "&regno=" + conn.GetFieldValue(0 , 0) + "&tc=" + tc);
					}
					else
