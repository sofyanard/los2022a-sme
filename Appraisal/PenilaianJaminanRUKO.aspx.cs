...
{
	/// <summary>
	/// Summary description for arRUKO.
	/// </summary>
	public partial class PenilaianJaminanRUKO : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				GlobalTools.fillRefList(ddl_AR_BANGUNAN,"select * from RF_APPR_JENISBANGUNAN where ACTIVE = '1'",false,conn);
		...
		private void ViewData()
		{
			...
			try
			{
				...
				try{ddl_AR_BANGUNAN.SelectedValue = conn.GetFieldValue("AR_BANGUNAN");} catch{}
				...
				txt_AR_BKNO.Text = conn.GetFieldValue("AR_BKNO");
				...
				txt_AR_IJNOAKTA.Text = conn.GetFieldValue("AR_IJNOAKTA");
				txt_AR_IJNOTARIS.Text = conn.GetFieldValue("AR_IJNOTARIS");
				txt_AR_IJSERTIFIKAT.Text = conn.GetFieldValue("AR_IJSERTIFIKAT");
				...
				try{ddl_Tujuan.SelectedValue = conn.GetFieldValue("AR_TUJUAN");} catch{}
		...
		protected void btn_Save_Click(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SP_APPR_RUKO 'Save','" + 
				lbl_AP_REGNO.Text + "','" + 
				lbl_CU_REF.Text + "','" + 
				lbl_CL_SEQ.Text + "','" +
				...
				txt_AR_JALAN.Text + "'," +
				GlobalTools.ConvertNull(txt_AR_KETJALAN.Text) + ",'" + 
				...
				txt_AR_IJNOAKTA.Text + "','" + 
				txt_AR_IJSERTIFIKAT.Text + "'," +
...
//				{
					...
					conn.QueryString = "exec APPRAISAL_UPDATESTATUS '" +lbl_AP_REGNO.Text+ "', '"+lbl_CU_REF.Text+"', " +lbl_CL_SEQ.Text+ ", '"+
						LA_APPRSTATUS.ToString()+"', "+ GlobalTools.ConvertFloat(txt_AR_HRGWAJAR.Text)+", "+
						GlobalTools.ConvertFloat(txt_AR_HRGPASAR.Text)+", '" +TABLENAME.Trim()+ "', '"+mStat.ToString().Trim()+"'";
					conn.ExecuteQuery();
...
//				SafetyMargin = 0.75;
			...
			// safety margin RUKO sama denagn TANAH
			// edit by Fajar 24-10-2005
			if (Score <= 5)
				SafetyMargin = 0.10;
			else if (Score == 6)
				SafetyMargin = 0.15;
		...
		private void DisabledEntry()
		{
			...
			txt_AR_IJNOAKTA.ReadOnly = true;
			txt_AR_IJNOTARIS.ReadOnly = true;
			txt_AR_IJSERTIFIKAT.ReadOnly = true;
		...
		{
			//-----------------------------------------------------------------delete dari SP_APPR_RUKO
			conn.QueryString = "SP_APPR_RUKO 'Delete','" + 
				lbl_AP_REGNO.Text + "','" + 
				lbl_CU_REF.Text + "','" + 
				lbl_CL_SEQ.Text + "'";
				conn.ExecuteNonQuery();
			
			//-----------------------------------------------------------------delete dari APPR_LIST
