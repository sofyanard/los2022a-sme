...
{
	/// <summary>
	/// Summary description for PenilaianJaminanSawah.
	/// </summary>
	public partial class PenilaianJaminanSawah : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				GlobalTools.fillRefList(ddl_AT_SJARAKPMK,"select * from RF_APPR_JARAK where ACTIVE = '1'",false,conn);
				GlobalTools.fillRefList(ddl_AT_KUBURAN,"select * from RF_APPR_JARAK where ACTIVE = '1'",false,conn);

				GlobalTools.fillRefList(ddl_AT_WILAYAH,"select * from RF_APPR_WILAYAH where ACTIVE = '1'",false,conn);
		...
		private void ViewData()
		{
			...
			try
			{
				...
				try {GlobalTools.fillDateForm(txt_AT_APPRDATE_dd,ddl_AT_APPRDATE_mm,txt_AT_APPRDATE_yy,Convert.ToDateTime(conn.GetFieldValue("AT_APPRDATE")));} catch {}
				...
				try{ddl_AT_LISTRIK.SelectedValue = conn.GetFieldValue("AT_LISTRIK");} catch {}
				try{ddl_AT_TELP.SelectedValue = conn.GetFieldValue("AT_TELP");} catch {}
				txt_AT_SIRIGASI.Text = conn.GetFieldValue("AT_SIRIGASI");
				txt_AT_SWILUTK.Text = conn.GetFieldValue("AT_SWILUTK");
				
				try{ddl_AT_STNHPRODUKIF.SelectedValue = conn.GetFieldValue("AT_STNHPRODUKIF");} catch {}
				try{ddl_AT_SJARAKPMK.SelectedValue = conn.GetFieldValue("AT_SJARAKPMK");} catch {}
				try{ddl_AT_SPBU.SelectedValue = conn.GetFieldValue("AT_SPBU");} catch {}
				try{ddl_AT_IBADAH.SelectedValue = conn.GetFieldValue("AT_IBADAH");} catch {}
				try{ddl_AT_HIBURAN.SelectedValue = conn.GetFieldValue("AT_HIBURAN");} catch {}
				try{ddl_AT_KUBURAN.SelectedValue = conn.GetFieldValue("AT_KUBURAN");} catch {}
				try{ddl_AT_ISNEW.SelectedValue = conn.GetFieldValue("AT_ISNEW");} catch {}
				txt_AT_THNBELI.Text = GlobalTools.ConvertFloat(conn.GetFieldValue("AT_THNBELI"));
				try{ddl_AT_WILAYAH.SelectedValue = conn.GetFieldValue("AT_WILAYAH");} catch {}
				
				try{ddl_AT_BANJIR.SelectedValue = conn.GetFieldValue("AT_BANJIR");} catch {}
				try{ddl_AT_TEGANGAN.SelectedValue = conn.GetFieldValue("AT_TEGANGAN");} catch {}
				try{ddl_AT_TNHLONGSOR.SelectedValue = conn.GetFieldValue("AT_TNHLONGSOR");} catch {}
				try{ddl_AT_PENCEMARAN.SelectedValue = conn.GetFieldValue("AT_PENCEMARAN");} catch {}
				txt_AT_KUALITAS.Text = conn.GetFieldValue("AT_KUALITAS");
				try{ddl_AT_KONTURTNH.SelectedValue = conn.GetFieldValue("AT_KONTURTNH");} catch {}
				try{ddl_AT_SWILAYAH.SelectedIndex = Convert.ToByte(conn.GetFieldValue("AT_SWILAYAH"));} catch {}
				try{ddl_AT_SLOKASI.SelectedIndex = Convert.ToByte(conn.GetFieldValue("AT_SLOKASI"));} catch {}
				try{ddl_AT_SKUALITAS.SelectedIndex = Convert.ToByte(conn.GetFieldValue("AT_SKUALITAS"));} catch {}
				try{ddl_AT_SLINGKUNGAN.SelectedIndex = Convert.ToByte(conn.GetFieldValue("AT_SLINGKUNGAN"));} catch {}

				//=====================================================================
				try{ddl_AT_BKTYPE.SelectedValue = conn.GetFieldValue("AT_BKTYPE");} catch {}
				txt_AT_BKNO.Text = conn.GetFieldValue("AT_BKNO");
				try {GlobalTools.fillDateForm(txt_AT_BKDATE_dd,ddl_AT_BKDATE_mm,txt_AT_BKDATE_yy,Convert.ToDateTime(conn.GetFieldValue("AT_BKDATE")));}	catch {}
				...
				//=====================================================================
				try{ddl_AT_IJTYPE.SelectedValue = conn.GetFieldValue("AT_IJTYPE");} catch {}
				...
				try {GlobalTools.fillDateForm(txt_AT_ABDATE_dd,ddl_AT_ABDATE_mm,txt_AT_ABDATE_yy,Convert.ToDateTime(conn.GetFieldValue("AT_ABDATE")));} catch {}
				...
				//=====================================================================
				try{ddl_Tujuan.SelectedValue = conn.GetFieldValue("AT_TUJUAN");} catch {}
				try{ddl_AT_MARKET.SelectedValue = conn.GetFieldValue("AT_MARKET");} catch {}
				try{ddl_AT_IKAT.SelectedValue = conn.GetFieldValue("AT_IKAT");} catch {}
				try{ddl_AT_KUASA.SelectedValue = conn.GetFieldValue("AT_KUASA");} catch {}
				try{ddl_AT_MASALAH.SelectedValue = conn.GetFieldValue("AT_MASALAH");} catch {}
		...
		protected void btn_Save_Click(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SP_APPR_SAWAH 'Save','" + lbl_AP_REGNO.Text + "','" + lbl_CU_REF.Text + "'," +
				lbl_CL_SEQ.Text + ",'" +
				...
				txt_AT_SIRIGASI.Text + "','" +
				ddl_AT_STNHPRODUKIF.SelectedValue + "','" +
				ddl_AT_SJARAKPMK.SelectedValue + "','" +
...
//				}
		...
		private int HitungScore()
		{
			...
			txt_AT_SWILUTK.Text = ddl_AT_SWILAYAH.SelectedValue.ToString();
			txt_AT_JALAN.Text = ddl_AT_SLOKASI.SelectedValue.ToString();
			txt_AT_KUALITAS.Text = ddl_AT_SKUALITAS.SelectedValue.ToString();
			txt_AT_SIRIGASI.Text = ddl_AT_SLINGKUNGAN.SelectedValue.ToString();
			...
			try {nBangunan = Convert.ToByte(ddl_AT_SLINGKUNGAN.SelectedItem.Text);}
			catch {nBangunan = 0;}
		...
		private void DisabledEntry()
		{
			...
			txt_AT_SIRIGASI.ReadOnly = true;
			txt_AT_SWILUTK.ReadOnly = true;
			ddl_AT_STNHPRODUKIF.Enabled = false;
			ddl_AT_SJARAKPMK.Enabled = false;
		...
		{
			//-----------------------------------------------------------------hapus dari APPR_SAWAH
			conn.QueryString = "SP_APPR_SAWAH 'Delete','" + 
				lbl_AP_REGNO.Text + "','"+ 
				lbl_CU_REF.Text + "','" +				
				lbl_CL_SEQ.Text + "'";
			conn.ExecuteNonQuery();
