...
{
	/// <summary>
	/// Summary description for TradeTransaction.
	/// </summary>
	public partial class TradeTransaction : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				ViewData();
				LBL_TEMPLATE.Text = "DD-FACILITIES-TRADETRANS";
		...
		{
			conn.QueryString = "select * from DD_FIELDS_TRADETRANSACTION where is_active='1'";
			conn.ExecuteQuery();
			FillGrid();
		}
		...
		private void SearchDataField()
		{
			...
			{
				conn.QueryString = "select * from DD_FIELDS_TRADETRANSACTION where fieldsname like '%"+ TXT_FIELD_NAME.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
		...
		private void SearchDataDesc()
		{
			if (TXT_DESC.Text!= "")
			{
				conn.QueryString = "select * from DD_FIELDS_TRADETRANSACTION where fieldsdescription like '%"+ TXT_DESC.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
