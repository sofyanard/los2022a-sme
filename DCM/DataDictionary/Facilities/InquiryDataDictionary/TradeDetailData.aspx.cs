...
{
	/// <summary>
	/// Summary description for TradeDetailData.
	/// </summary>
	public partial class TradeDetailData : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				ViewData();
				ViewData();
				LBL_TEMPLATE.Text = "DD-FACILITIES-TRADE";
		...
		{
			conn.QueryString = "select * from DD_FIELDS_TRADE where is_active='1'";
			conn.ExecuteQuery();
			FillGrid();
		}
		...
		private void SearchDataField()
		{
			...
			{
				conn.QueryString = "select * from DD_FIELDS_TRADE where fieldsname like '%"+ TXT_FIELD_NAME.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
		...
		private void SearchDataDesc()
		{
			if (TXT_DESC.Text!= "")
			{
				conn.QueryString = "select * from DD_FIELDS_TRADE where fieldsdescription like '%"+ TXT_DESC.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
