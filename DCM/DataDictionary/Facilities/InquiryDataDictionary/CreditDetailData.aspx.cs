...
{
	/// <summary>
	/// Summary description for CreditDetailData.
	/// </summary>
	public partial class CreditDetailData : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				ViewData();
				LBL_TEMPLATE.Text = "DD-FACILITIES-CREDIT";
		...
		{
			conn.QueryString = "select * from DD_FIELDS_PERKREDITAN where is_active='1'";
			conn.ExecuteQuery();
			FillGrid();
		}
		...
		private void SearchDataField()
		{
			...
			{
				conn.QueryString = "select * from DD_FIELDS_PERKREDITAN where fieldsname like '%"+ TXT_FIELD_NAME.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
		...
		private void SearchDataDesc()
		{
			if (TXT_DESC.Text!= "")
			{
				conn.QueryString = "select * from DD_FIELDS_PERKREDITAN where fieldsdescription like '%"+ TXT_DESC.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
