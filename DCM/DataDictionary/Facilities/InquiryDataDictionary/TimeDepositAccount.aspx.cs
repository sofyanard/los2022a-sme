...
{
	/// <summary>
	/// Summary description for TimeDepositAccount.
	/// </summary>
	public partial class TimeDepositAccount : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				ViewData();
				LBL_TEMPLATE.Text = "DD-FACILITIES-TIME";
		...
		{
			conn.QueryString = "select * from DD_FIELDS_TIMEDEPOSITTRANSACTION where is_active='1'";
			conn.ExecuteQuery();
			FillGrid();
		}
		...
		private void SearchDataField()
		{
			...
			{
				conn.QueryString = "select * from DD_FIELDS_TIMEDEPOSITTRANSACTION where fieldsname like '%"+ TXT_FIELD_NAME.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
		...
		private void SearchDataDesc()
		{
			if (TXT_DESC.Text!= "")
			{
				conn.QueryString = "select * from DD_FIELDS_TIMEDEPOSITTRANSACTION where fieldsdescription like '%"+ TXT_DESC.Text +"%' and is_active='1'";
				conn.ExecuteQuery();
			}
			else
			{
