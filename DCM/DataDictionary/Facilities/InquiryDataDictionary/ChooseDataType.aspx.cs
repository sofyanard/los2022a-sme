...
namespace SME.DCM.DataDictionary.Facilities.InquiryDataDictionary
{
	...
	/// </summary>
	public partial class ChooseDataType : System.Web.UI.Page
	{
		protected System.Web.UI.HtmlControls.HtmlTableRow TR_BUC_DANA;
		protected Connection conn = new Connection(ConfigurationSettings.AppSettings["conn2"]);
	
		protected void Page_Load(object sender, System.EventArgs e)
		{
			//Response.Redirect("InquiryDataAll.aspx",true);
			if(!IsPostBack)
			{
		...
		private void setTableItem()
		{
			...
			try
			{
				conn.QueryString = "SELECT * FROM VW_DCM_CHOOSE_DATA_TYPE";
				conn.ExecuteQuery();
			}
			catch (Exception a)
			{
				string b = a.Message.ToString();
				string c = a.Message.ToString();
			}
		...
		private void CheckingClicked(Control Page)
		{
			foreach (Control ctrl in Page.Controls)
			{
				if(ctrl is HtmlInputRadioButton)
				{
					...
					if(ctrlfix.Checked)
					{
						//Tools.popMessage(this, ctrlfix.Value.ToString());
						try
						{
							Response.Redirect("InquiryDataAll.aspx",true);
						}
