...
{
	/// <summary>
	/// Summary description for InquiryRequestData.
	/// </summary>
	public partial class InquiryRequestData : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				DDL_REQ_MONTH1.Items.Add(new ListItem("-- Pilih --",""));
				DDL_REQ_MONTH2.Items.Add(new ListItem("-- Pilih --",""));
				DDL_RESULT_MONTH1.Items.Add(new ListItem("-- Pilih --",""));
				DDL_RESULT_MONTH2.Items.Add(new ListItem("-- Pilih --",""));
				for (int i = 1; i <= 12; i++)
				{
					DDL_REQ_MONTH1.Items.Add(new ListItem(DateAndTime.MonthName(i, false), i.ToString()));
					DDL_REQ_MONTH2.Items.Add(new ListItem(DateAndTime.MonthName(i, false), i.ToString()));
					DDL_RESULT_MONTH1.Items.Add(new ListItem(DateAndTime.MonthName(i, false), i.ToString()));
					DDL_RESULT_MONTH2.Items.Add(new ListItem(DateAndTime.MonthName(i, false), i.ToString()));
				}
				FillDDLType();
			}
			ViewExportFiles();
		...
		{
			conn.QueryString = "SELECT DISTINCT * FROM VW_SDC_FACILITIES_INQUIRY_REQ_DATA ORDER BY SEQ";
		...
		private void FillDDLType()
		{
			...
			conn.QueryString = "SELECT DISTINCT TEMPLATE_ID, TEMPLATE_DESC FROM DOCEXPORT_TEMPLATE_MASTER WHERE TEMPLATE_GROUP = 'SDCFACILITIESREQDOC1'";
			conn.ExecuteQuery();
		...
		{
			conn.QueryString = "SELECT * FROM VW_DOCEXPORT_VIEWFILEEXPORT WHERE TEMPLATE_GROUP = 'SDCFACILITIESREQDOC' AND FE_USERID = '" + Session["UserID"].ToString() + "'";
			conn.ExecuteQuery();
		...
		private void SearchData()
		{
			...
			if (TXT_REQ_DAY1.Text != "" && DDL_REQ_MONTH1.SelectedValue != "" && TXT_REQ_YEAR1.Text != "" && TXT_REQ_DAY2.Text != "" && DDL_REQ_MONTH2.SelectedValue != "" && TXT_REQ_YEAR2.Text != "") 
			{
				if (!GlobalTools.isDateValid(TXT_REQ_DAY1.Text, DDL_REQ_MONTH1.SelectedValue, TXT_REQ_YEAR1.Text)) 
				{
					GlobalTools.popMessage(this, "Date Not Valid!");
					return;
				}

				if (!GlobalTools.isDateValid(TXT_REQ_DAY2.Text, DDL_REQ_MONTH2.SelectedValue, TXT_REQ_YEAR2.Text)) 
				{
					GlobalTools.popMessage(this, "Date Not Valid!");
					return;
				}
				...
				{
					query += "AND REQ_DATE BETWEEN " + tools.ConvertDate(TXT_REQ_DAY1.Text, DDL_REQ_MONTH1.SelectedValue, TXT_REQ_YEAR1.Text) + " AND " + tools.ConvertDate(TXT_REQ_DAY2.Text, DDL_REQ_MONTH2.SelectedValue, TXT_REQ_YEAR2.Text) + " ";
				}
			}

			if (TXT_RESULT_DAY1.Text != "" && DDL_RESULT_MONTH1.SelectedValue != "" && TXT_RESULT_YEAR1.Text != "" && TXT_RESULT_DAY2.Text != "" && DDL_RESULT_MONTH2.SelectedValue != "" && TXT_RESULT_YEAR2.Text != "") 
			{
				if (!GlobalTools.isDateValid(TXT_RESULT_DAY1.Text, DDL_RESULT_MONTH1.SelectedValue, TXT_RESULT_YEAR1.Text)) 
				{
					GlobalTools.popMessage(this, "Date Not Valid!");
					return;
				}

				if (!GlobalTools.isDateValid(TXT_RESULT_DAY2.Text, DDL_RESULT_MONTH2.SelectedValue, TXT_RESULT_YEAR2.Text)) 
				{
					GlobalTools.popMessage(this, "Date Not Valid!");
					return;
				}
				...
				{
					query += "AND REQ_DATE BETWEEN " + tools.ConvertDate(TXT_RESULT_DAY1.Text, DDL_RESULT_MONTH1.SelectedValue, TXT_RESULT_YEAR1.Text) + " AND " + tools.ConvertDate(TXT_RESULT_DAY2.Text, DDL_RESULT_MONTH2.SelectedValue, TXT_RESULT_YEAR2.Text) + " ";
				}
			}

			if(query != "")
			{
				conn.QueryString = "SELECT DISTINCT * FROM VW_SDC_FACILITIES_INQUIRY_REQ_DATA WHERE 1=1 " + query + "ORDER BY SEQ";
				BindData(DATA_GRID.ID.ToString(), conn.QueryString);
			}
				...
				try
				{
					...
					if (dt1.Rows.Count > 0)
					{
						for (int i = 0; i < dt1.Rows.Count; i++)
						{
							...
							conn.QueryString = "EXEC " + proc + " " + tools.ConvertDate(TXT_REQ_DAY1.Text, DDL_REQ_MONTH1.SelectedValue, TXT_REQ_YEAR1.Text) + "," +
												tools.ConvertDate(TXT_RESULT_DAY1.Text, DDL_RESULT_MONTH1.SelectedValue, TXT_RESULT_YEAR1.Text);
							conn.ExecuteQuery();
