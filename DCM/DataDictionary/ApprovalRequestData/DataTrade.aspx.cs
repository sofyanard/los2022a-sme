...
{
	/// <summary>
	/// Summary description for DataTrade.
	/// </summary>
	public partial class DataTrade : System.Web.UI.Page
	{
		...
		private void ViewData()
		{
			...
			if(FIELDS_NAME == "" && DESC == "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_TRADE");
			}
			else if(FIELDS_NAME != "" && DESC == "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_TRADE WHERE FIELDSNAME like '%" + FIELDS_NAME + "%'");
			}
			else if(FIELDS_NAME == "" && DESC != "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_TRADE WHERE FIELDSDESCRIPTION like '%" + DESC + "%'");
			}
			else if(FIELDS_NAME != "" && DESC != "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_TRADE WHERE FIELDSDESCRIPTION like '%" + DESC + "%' AND FIELDSNAME like '%" +  FIELDS_NAME+ "%'");
			}
			setUpeventhandlerCB();
		...
		private void BindCBWithExistingData()
		{
			for (int i = 0; i < DATA_GRID.Items.Count; i++)
			{
				conn.QueryString = "SELECT * FROM DD_REQUESTED_FIELD_LIST WHERE AP_REGNO = '" + Request.QueryString["regno"] + "' AND FIELDSNAME = '" + DATA_GRID.Items[i].Cells[0].Text.ToString() + "' AND TABLE_NAME = 'DD_FIELDS_TRADE'";
				conn.ExecuteQuery();
		...
		private void CB_STATUS_CheckedChanged(object sender, EventArgs e)
		{
			...
			if(b.Checked == true)
			{
				conn.QueryString = "EXEC DD_INSERT_REQUESTED_FIELD_LIST '" + Request.QueryString["regno"] + "','" + Session["UserID"] + "','DD_FIELDS_TRADE','" + idx + "','INSERT'" ;
			}
			else
			{
				conn.QueryString = "EXEC DD_INSERT_REQUESTED_FIELD_LIST '" + Request.QueryString["regno"] + "','" + Session["UserID"] + "','DD_FIELDS_TRADE','" + idx + "','DELETE'" ;
			}
			conn.ExecuteQuery();
		}
