...
{
	/// <summary>
	/// Summary description for RequestApprList.
	/// </summary>
	public partial class RequestApprList : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_DD_APPROVAL_REQUEST_LIST WHERE DSO_APPROVER = '" + Session["UserID"] + "'"; 
			conn.ExecuteQuery();
		...
		private void BindDataRadioButtonList()
		{
			for(int i=0; i<DGR_REQUESTLIST.Items.Count; i++)
			{
				try
				{
					...
					conn.QueryString = "SELECT * FROM VW_DD_APPROVAL_REQUEST_LIST WHERE REQ_NUMBER = '" + DGR_REQUESTLIST.Items[i].Cells[1].Text.Trim() + "'";
					conn.ExecuteQuery();
