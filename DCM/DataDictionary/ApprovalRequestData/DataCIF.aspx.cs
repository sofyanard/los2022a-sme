...
{
	/// <summary>
	/// Summary description for DataCIF.
	/// </summary>
	public partial class DataCIF : System.Web.UI.Page
	{
		...
		{
			//ViewData(TXT_FIELD_NAME.Text.ToString(), "");
			ViewData();
		}

		protected void BTN_FIND_DESC_Click(object sender, System.EventArgs e)
		{
			//ViewData("", TXT_DESCRIPTION.Text.ToString());
			ViewData();
		}
		...
		private void ViewData()
		{
			/*if(FIELDS_NAME == "" && DESC == "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_CIF");
			}
			else if(FIELDS_NAME != "" && DESC == "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_CIF WHERE FIELDSNAME like '%" + FIELDS_NAME + "%'");
			}
			else if(FIELDS_NAME == "" && DESC != "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_CIF WHERE FIELDSDESCRIPTION like '%" + DESC + "%'");
			}
			else if(FIELDS_NAME != "" && DESC != "")
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_CIF WHERE FIELDSDESCRIPTION like '%" + DESC + "%' AND FIELDSNAME = '%" +  FIELDS_NAME+ "%'");
			}*/
			...
			{
				BindData("DATA_GRID","SELECT * FROM DD_FIELDS_CIF WHERE FIELDSDESCRIPTION like '%" + DESC + "%' AND FIELDSNAME like '%" +  FIELDS_NAME+ "%'");
			}
			setUpeventhandlerCB();
		...
		private void BindCBWithExistingData()
		{
			for (int i = 0; i < DATA_GRID.Items.Count; i++)
			{
				conn.QueryString = "SELECT * FROM DD_REQUESTED_FIELD_LIST WHERE AP_REGNO = '" + Request.QueryString["regno"] + "' AND FIELDSNAME = '" + DATA_GRID.Items[i].Cells[0].Text.ToString() + "' AND TABLE_NAME = 'DD_FIELDS_CIF'";
				conn.ExecuteQuery();
		...
		private void CB_STATUS_CheckedChanged(object sender, EventArgs e)
		{
			...
			if(b.Checked == true)
			{
				conn.QueryString = "EXEC DD_INSERT_REQUESTED_FIELD_LIST '" + Request.QueryString["regno"] + "','" + Session["UserID"] + "','DD_FIELDS_CIF','" + idx + "','INSERT'" ;
			}
			else
			{
				conn.QueryString = "EXEC DD_INSERT_REQUESTED_FIELD_LIST '" + Request.QueryString["regno"] + "','" + Session["UserID"] + "','DD_FIELDS_CIF','" + idx + "','DELETE'" ;
			}
			conn.ExecuteQuery();
		}
