...
{
	/// <summary>
	/// Summary description for AssignMain.
	/// </summary>
	public partial class AssignMain : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_DD_ASSSIGNMENT_VALIDATION_LIST WHERE REQ_NUMBER = '" + Request.QueryString["regno"] + "'";
			conn.ExecuteQuery();

			switch(conn.GetFieldValue("DATA_FLAG").ToString())
			{
				case "0":
					IMG_ASSIGN_STATUS.ImageUrl = "../../../image/UnComplete.gif";
					LBL_ASSIGN_STATUS.Text = "Not Assign";
					break;

				case "1":
					IMG_ASSIGN_STATUS.ImageUrl = "../../../image/UnComplete.gif";
					LBL_ASSIGN_STATUS.Text = "Assign";
					break;

				case "2":
					IMG_ASSIGN_STATUS.ImageUrl = "../../../image/UnComplete.gif";
					LBL_ASSIGN_STATUS.Text = "In Process";
					break;

				case "3":
					IMG_ASSIGN_STATUS.ImageUrl = "../../../image/Complete.gif";
					LBL_ASSIGN_STATUS.Text = "Status Done";
		...
		{
			DDL_PICSERVICE.Items.Clear();
			DDL_PICSERVICE.Items.Add(new ListItem("--Select--", ""));

			conn.QueryString = "SELECT * FROM VW_DD_RF_PIC_DSO";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
				DDL_PICSERVICE.Items.Add(new ListItem(conn.GetFieldValue(i,1), conn.GetFieldValue(i,0)));
			}
		}

		private void ViewData()
		{
			conn.QueryString = "SELECT REQ_NUMBER,REQ_PURPOSE,CONVERT(VARCHAR,REQ_DATE,106) AS REQ_DATE,REQ_REMARK,REQ_PIC,REQ_FILE_RESULT FROM DD_DATA_REQUEST WHERE REQ_NUMBER = '" + Request.QueryString["regno"] + "'";
			conn.ExecuteQuery();
			...
			DDL_PICSERVICE.SelectedValue= conn.GetFieldValue("REQ_PIC").ToString();
		...
		protected void BTN_UPLOAD1_Click(object sender, System.EventArgs e)
		{
			string directory;
			int counter = 0;
			int max_id = 10000;
			...
			conn.QueryString = "EXEC DD_INSERT_FILE_EXP '2','" + 
								Request.QueryString["regno"] + "','" +
								Session["UserID"].ToString() + "','" +
								Path.GetFileName(TXT_FILE_UPLOAD1.PostedFile.FileName) + "'";
			conn.ExecuteQuery();
			...
			{
				outputfilename = conn.GetFieldValue("FILE_UPLOAD_EXP_NAME");
				TXT_XLSNAME1.Text = outputfilename;
			}
		...
		protected void BTN_ASSIGN_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if(conn.GetFieldValue("AP_CURRTRACK").ToString() == "DD3.0")
				{
				...
				conn.QueryString = "UPDATE DD_DATA_REQUEST SET DSO_PIC = '" + DDL_PICSERVICE.SelectedValue + "', DATA_FLAG = '1' WHERE REQ_NUMBER = '" + Request.QueryString["regno"] + "'";
				conn.ExecuteQuery();
		...
		protected void BTN_REASSIGN_Click(object sender, System.EventArgs e)
		{
			try
			{
				conn.QueryString = "SELECT * FROM DD_APPTRACK WHERE AP_REGNO = '" + Request.QueryString["regno"] + "'";
				...
				conn.QueryString = "UPDATE DD_DATA_REQUEST SET REQ_PIC = '" + DDL_PICSERVICE.SelectedValue + "', DATA_FLAG = '1' WHERE REQ_NUMBER = '" + Request.QueryString["regno"] + "'";
				conn.ExecuteQuery();
		...
		{
			Response.Redirect("DataAssignValidList.aspx?mc=" + Request.QueryString["mc"]);
		}
	}
}
