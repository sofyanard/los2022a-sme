...
{
	/// <summary>
	/// Summary description for DataTypeAppr.
	/// </summary>
	public partial class DataTypeAppr : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				BindData("DGR_PROBAPPR","SELECT * FROM DD_DATA_TYPE_TEMP");
			}
		}
		...
		protected void BTN_SUB_Click(object sender, System.EventArgs e)
		{
			for (int i=0; i<DGR_PROBAPPR.Items.Count; i++)
			{
				...
				{
					conn.QueryString = "EXEC DD_EXECUTE_PENDING_PARAMETER_DDL '" + DGR_PROBAPPR.Items[i].Cells[0].Text.ToString() + "','DATAPURPOSE','ACCEPT'";
					conn.ExecuteQuery();
				}
				else if(reject.Checked == true)
				{
					conn.QueryString = "EXEC DD_EXECUTE_PENDING_PARAMETER_DDL '" + DGR_PROBAPPR.Items[i].Cells[0].Text.ToString() + "','DATAPURPOSE','REJECT'";
					conn.ExecuteQuery();
				}
			}

			BindData("DGR_PROBAPPR","SELECT * FROM DD_DATA_PURPOSE_TEMP");
		}
