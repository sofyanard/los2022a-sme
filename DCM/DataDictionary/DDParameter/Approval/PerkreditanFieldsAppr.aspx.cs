...
{
	/// <summary>
	/// Summary description for PerkreditanFieldsAppr.
	/// </summary>
	public partial class PerkreditanFieldsAppr : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				BindData("DGR_PROBAPPR","SELECT * FROM DD_FIELDS_PERKREDITAN_TEMP");
			}
		}
		...
		protected void BTN_SUB_Click(object sender, System.EventArgs e)
		{
			for (int i=0; i<DGR_PROBAPPR.Items.Count; i++)
			{
				...
				{
					conn.QueryString = "EXEC DD_EXECUTE_PENDING_PARAMETER '" + DGR_PROBAPPR.Items[i].Cells[0].Text.ToString() + "','PERKREDITAN','ACCEPT'";
					conn.ExecuteQuery();
				}
				else if(reject.Checked == true)
				{
					conn.QueryString = "EXEC DD_EXECUTE_PENDING_PARAMETER '" + DGR_PROBAPPR.Items[i].Cells[0].Text.ToString() + "','PERKREDITAN','REJECT'";
					conn.ExecuteQuery();
				}
			}
