...
{
	/// <summary>
	/// Summary description for TradeTransAppr.
	/// </summary>
	public partial class TradeTransAppr : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				BindData("DGR_PROBAPPR","SELECT * FROM DD_FIELDS_TRADETRANSACTION_TEMP");
			}
		}
		...
		protected void BTN_SUB_Click(object sender, System.EventArgs e)
		{
			for (int i=0; i<DGR_PROBAPPR.Items.Count; i++)
			{
				...
				{
					conn.QueryString = "EXEC DD_EXECUTE_PENDING_PARAMETER '" + DGR_PROBAPPR.Items[i].Cells[0].Text.ToString() + "','TRADETRANSACTION','ACCEPT'";
					conn.ExecuteQuery();
				}
				else if(reject.Checked == true)
				{
					conn.QueryString = "EXEC DD_EXECUTE_PENDING_PARAMETER '" + DGR_PROBAPPR.Items[i].Cells[0].Text.ToString() + "','TRADETRANSACTION','REJECT'";
					conn.ExecuteQuery();
				}
			}
