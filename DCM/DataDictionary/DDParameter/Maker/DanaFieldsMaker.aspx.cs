...
{
	/// <summary>
	/// Summary description for DanaFieldsMaker.
	/// </summary>
	public partial class DanaFieldsMaker : System.Web.UI.Page
	{
		...
		{
			BindData("Dgr_CurrStage", "SELECT * FROM VW_DD_FIELDS_DANA");
		}

		private void ViewRequestingParameter()
		{
			BindData("DgRequesting", "SELECT * FROM VW_DD_FIELDS_DANA_TEMP");
		}
		...
		{
			conn.QueryString = "SELECT * FROM DD_FIELDS_DANA WHERE CODE = '" + TXT_CODE.Text + "'";
			conn.ExecuteQuery();
			...
			conn.QueryString = "EXEC DD_INSERT_PARAMETER 'DANA','" + TXT_CODE.Text + "','" + TXT_FIELDS_NAME.Text + "','" + TXT_FIELDS_DESCRIPTION.Text + "',1, '" + status + "'";
			conn.ExecuteQuery();
			ViewExisitingParameter();
			ViewRequestingParameter();
		...
		private void Dgr_CurrStage_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "edit":
					...
					break;
				case "delete":
					conn.QueryString = "EXEC DD_INSERT_PARAMETER 'DANA','" + e.Item.Cells[0].Text.ToString() + "','" + e.Item.Cells[1].Text.ToString() + "','" + e.Item.Cells[2].Text.ToString() + "',0, 'DELETE'";
					conn.ExecuteQuery();
					ViewExisitingParameter();
					ViewRequestingParameter();
					break;
				case "undelete":
					conn.QueryString = "EXEC DD_INSERT_PARAMETER 'DANA','" + e.Item.Cells[0].Text.ToString() + "','" + e.Item.Cells[1].Text.ToString() + "','" + e.Item.Cells[2].Text.ToString() + "',1, 'UNDELETE'";
					conn.ExecuteQuery();
					ViewExisitingParameter();
					ViewRequestingParameter();
					break;
		...
		private void DgRequesting_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "edit":
					...
					break;
				case "delete":
					conn.QueryString = "DELETE DD_FIELDS_DANA_TEMP WHERE CODE = '" + e.Item.Cells[0].Text + "'";
					conn.ExecuteQuery();
					ViewRequestingParameter();
					break;
			}
