...
{
	/// <summary>
	/// Summary description for DataPurposeMaker.
	/// </summary>
	public partial class DataPurposeMaker : System.Web.UI.Page
	{
		...
		{
			BindData("Dgr_CurrStage", "SELECT * FROM VW_DD_DATA_PURPOSE");
		}

		private void ViewRequestingParameter()
		{
			BindData("DgRequesting", "SELECT * FROM VW_DD_DATA_PURPOSE_TEMP");
		}
		...
		private void MakeVisibleDeleteUndelete(System.Web.UI.WebControls.DataGrid dg)
		{
			...
			{
				string a = dg.Items[i].Cells[2].Text.ToString();
				int num = int.Parse(dg.Items[i].Cells[2].Text.ToString());

				LinkButton toBeHidden;
				if(num == 1)
				{
					toBeHidden = ((LinkButton)dg.Items[i].Cells[3].FindControl("LNK_UNDELETE"));
					toBeHidden.Visible = false;
				}
				else if(num == 0)
				{
					toBeHidden = ((LinkButton)dg.Items[i].Cells[3].FindControl("LNK_DELETE"));
					toBeHidden.Visible = false;
				}
			}
		}
		...
		{
			conn.QueryString = "SELECT * FROM DD_DATA_PURPOSE WHERE CODE = '" + TXT_CODE.Text + "'";
			conn.ExecuteQuery();
			...
			conn.QueryString = "EXEC DD_INSERT_PARAMETER_DDL 'DATAPURPOSE','" + TXT_CODE.Text + "','" + TXT_DESCRIPTION.Text + "',1, '" + status + "'";
			conn.ExecuteQuery();
			ViewExisitingParameter();
			ViewRequestingParameter();
		...
		private void Dgr_CurrStage_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "edit":
					...
					TXT_DESCRIPTION.Text = e.Item.Cells[1].Text.ToString();
					break;
				case "delete":
					conn.QueryString = "EXEC DD_INSERT_PARAMETER_DDL 'DATAPURPOSE','" + e.Item.Cells[0].Text.ToString() + "','" + e.Item.Cells[1].Text.ToString() + "',0, 'DELETE'";
					conn.ExecuteQuery();
					ViewExisitingParameter();
					ViewRequestingParameter();
					break;
				case "undelete":
					conn.QueryString = "EXEC DD_INSERT_PARAMETER_DDL 'DATAPURPOSE','" + e.Item.Cells[0].Text.ToString() + "','" + e.Item.Cells[1].Text.ToString() + "',1, 'UNDELETE'";
					conn.ExecuteQuery();
					ViewExisitingParameter();
					ViewRequestingParameter();
					break;
		...
		private void DgRequesting_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "edit":
					...
					break;
				case "delete":
					conn.QueryString = "DELETE DD_DATA_PURPOSE_TEMP WHERE CODE = '" + e.Item.Cells[0].Text + "'";
					conn.ExecuteQuery();
					ViewRequestingParameter();
					break;
			}
