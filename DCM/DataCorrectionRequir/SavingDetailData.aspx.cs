...
{
	/// <summary>
	/// Summary description for SavingDetailData.
	/// </summary>
	public partial class SavingDetailData : System.Web.UI.Page
	{
		...
		private DQM.DDInquiry.body BodyRequest = new DQM.DDInquiry.body();
		private DQM.DDInquiry.DDAccountInquiryResponse ResponseClass = new DQM.DDInquiry.DDAccountInquiryResponse();

		private DQM.DDMaintenance.body BodyMaintenanceRequest = new DQM.DDMaintenance.body();
		private DQM.DDMaintenance.DDExistingAccDetailMaintenanceResponse ResponseMaintenanceClass = new DQM.DDMaintenance.DDExistingAccDetailMaintenanceResponse();
		
		private void RetrieveSOA()
		{
			BodyRequest.accountNumber = Request.QueryString["acctno"];
			ResponseClass = MainClass.DDInquiryService(BodyRequest);
		...
		*/
		...
		private void CheckError()
		{
			...
			for (int i = 0; i < conn2.GetRowCount(); i++)
			{
				...
				conn3.QueryString = "SELECT IDCONTROL FROM DCM_TABLE_ERROR WHERE PAGE = 'SavingDetailData.aspx' AND MESSAGE = '" + err_msg.Trim() + "'";
				conn3.ExecuteQuery();
		...
		{
			/*
			BodyMaintenanceRequest.accountNumber = "";
			BodyMaintenanceRequest.accountType = "";

			ResponseMaintenanceClass = MainClass.DDMaintenanceService(BodyMaintenanceRequest);
			RetrieveDataFromSOAResponse(ResponseMaintenanceClass);
			*/
		}

		/*
		public void RetrieveDataFromSOAResponse(DQM.DDMaintenance.DDExistingAccDetailMaintenanceResponse response)
		{
