...
{
	/// <summary>
	/// Summary description for CashCollateralDEP.
	/// </summary>
	public partial class CashCollateralDEP : System.Web.UI.Page
	{
		...
		private string VAR_NilaiAgunanBank;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				DDL_PENERBIT.Items.Add(new ListItem("--Pilih--", ""));
				...
				for (int i=1; i<=12; i++)
				{
					...
					DDL_JNS_PENGIKATAN.Items.Add(new ListItem(DateAndTime.MonthName(i, false), i.ToString()));	
				}
				...
				conn2.QueryString = "SELECT * FROM VW_DCM_COLLATERAL_DDL_PENERBIT_AGUNAN";
				conn2.ExecuteQuery();
				for (int i=0; i<conn2.GetRowCount(); i++)
					DDL_PENERBIT.Items.Add(new ListItem(conn2.GetFieldValue(i,1), conn2.GetFieldValue(i,0)));
		...
		private void CheckError()
		{
			...
			for (int i = 0; i < conn2.GetRowCount(); i++)
			{
				...
				conn3.QueryString = "SELECT IDCONTROL FROM DCM_TABLE_ERROR WHERE PAGE = 'CashCollateralDEP.aspx' AND MESSAGE = '" + err_msg.Trim() + "'";
				conn3.ExecuteQuery();
		...
		private void VAR_to_CONTROL()
		{
			...
			DDL_PENERBIT.SelectedValue			= VAR_PenerbitAgunan;
		    TXT_NILAI_AGUNAN.Text				= VAR_NilaiAgunanBank;
			TXT_NILAI_APPR.Text					= VAR_NilaiAppraisal;
			TXT_PENILAI.Text					= VAR_NamaPenilaiIndependen;
			...
			TXT_THN_PENGIKATAN.Text				= VAR_TanggalPengikatanYeaar;
		...
		private void CONTROL_to_VAR()
		{
			...
			VAR_PenerbitAgunan					= DDL_PENERBIT.SelectedValue;
			VAR_NilaiAgunanBank					= TXT_NILAI_AGUNAN.Text;
			VAR_NilaiAppraisal					= TXT_NILAI_APPR.Text;
			VAR_NamaPenilaiIndependen			= TXT_PENILAI.Text;
			...
			VAR_TanggalPengikatanYeaar			= TXT_THN_PENGIKATAN.Text;
