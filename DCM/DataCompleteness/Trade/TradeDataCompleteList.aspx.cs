...
namespace SME.DCM.DataCompleteness.Trade
{
	...
	/// </summary>
	public partial class TradeDataCompleteList : System.Web.UI.Page
	{
		...
		{
			conn2.QueryString = "SELECT DISTINCT * FROM VW_DCM_DATACOMPLETENESS_TRADE_TRADEDATACOMPLETELIST";
			conn2.ExecuteQuery();
		...
		private void DGR_INTERIM_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				case "edit_data":
					if(e.Item.Cells[3].Text.Trim() == "LC")
						Response.Redirect("LCDataComplet.aspx?cif_no=" + e.Item.Cells[1].Text.Trim());
					else if (e.Item.Cells[3].Text.Trim() == "BG")
						Response.Redirect("BGDataComplet.aspx?cif_no=" + e.Item.Cells[1].Text.Trim());
					else if (e.Item.Cells[3].Text.Trim() == "SJ")
						Response.Redirect("SetorJamDataComplet.aspx?cif_no=" + e.Item.Cells[1].Text.Trim());
					break;
			}
		}
		...
		private void SearchData()
		{
			...
			{
				query += "AND PRODUCT='" + TXT_REFERENCE.Text + "'";
			}
			...
			if(query != "")
			{
				conn2.QueryString = "SELECT DISTINCT * FROM VW_DCM_DATACOMPLETENESS_TRADE_TRADEDATACOMPLETELIST WHERE 1=1 " + query;
				conn2.ExecuteQuery();
				FillGrid();
			}
			else
			{
				conn2.QueryString = "SELECT DISTINCT * FROM VW_DCM_DATACOMPLETENESS_TRADE_TRADEDATACOMPLETELIST WHERE 1=1";
				conn2.ExecuteQuery();
				FillGrid();
			}
		}
