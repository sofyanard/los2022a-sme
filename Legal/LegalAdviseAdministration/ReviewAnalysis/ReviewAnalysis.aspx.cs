...
namespace SME.Legal.LegalAdviseAdministration.ReviewAnalysis
{
	...
	public partial class ReviewAnalysis : System.Web.UI.Page
	{
		...
		private void ViewData()
		{
			...
			if(conn.GetFieldValue("AP_CURRTRACK").ToString() == "LGL.3" && conn.GetFieldValue("PROCESS_STATUS").ToString() == "1")
			{
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				conn.QueryString = "UPDATE LGAM_LEGAL_ADMIN SET SUMMARY_ADVIS = '" + TXT_SUMMARY_ADVIS.Text + "', FINISH_DATE = " + TGL + ", PROCESS_STATUS = '1' WHERE CU_REF = '" + Request.QueryString["curef"] + "'";
				conn.ExecuteQuery();
