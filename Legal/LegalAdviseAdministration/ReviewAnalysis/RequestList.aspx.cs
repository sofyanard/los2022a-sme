...
namespace SME.Legal.LegalAdviseAdministration.ReviewAnalysis
{
	...
	public partial class RequestList : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_LGAM_REQUEST_LIST WHERE LEGAL_OFFICER = '" + Session["UserID"].ToString() + "' AND AP_CURRTRACK = 'LGL.3'";
		...
		private void SearchData()
		{
			...
			if(query != "")
			{
				conn.QueryString = "SELECT DISTINCT * FROM VW_LGAM_REQUEST_LIST WHERE 1=1 AND LEGAL_OFFICER = '" + Session["UserID"].ToString() + "' AND AP_CURRTRACK = 'LGL.3' " + query + "ORDER BY SEQ";
				BindData(DATA_GRID.ID.ToString(), conn.QueryString);
			}

			else
			{
				conn.QueryString = "SELECT DISTINCT * FROM VW_LGAM_REQUEST_LIST WHERE LEGAL_OFFICER = '" + Session["UserID"].ToString() + "' AND AP_CURRTRACK = 'LGL.3' ORDER BY SEQ";
				BindData(DATA_GRID.ID.ToString(), conn.QueryString);
			}
		}
	}
