...
namespace SME.Legal.LegalAdviseAdministration.Reporting
{
	/// <summary>
	/// Summary description for LegalAdminReporting.
	/// </summary>
	public partial class LegalAdminReporting : System.Web.UI.Page
	{
		...
		{
			DDL_UNIT_REQUEST.Items.Clear();
			DDL_UNIT_REQUEST.Items.Add(new ListItem("--Select--", ""));

			conn.QueryString = "SELECT * FROM VW_LGAM_DDLUNIT";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
				DDL_UNIT_REQUEST.Items.Add(new ListItem(conn.GetFieldValue(i,1), conn.GetFieldValue(i,0)));
			}
		}
		...
		private void FillDDLType()
		{
			...
			conn.QueryString = "SELECT DISTINCT TEMPLATE_ID, TEMPLATE_DESC FROM DOCEXPORT_TEMPLATE_MASTER WHERE TEMPLATE_GROUP = 'LGAMREPORTING'";
			conn.ExecuteQuery();
			for(int i=0; i < conn.GetRowCount(); i++)
			{
		...
		{
			conn.QueryString = "SELECT * FROM VW_DOCEXPORT_VIEWFILEEXPORT WHERE TEMPLATE_GROUP = 'LGAMREPORTING' AND FE_USERID = '" + Session["UserID"].ToString() + "'";
			conn.ExecuteQuery();
		...
		private void ClearData()
		{
			...
			TXT_THN2.Text					= "";
			DDL_UNIT_REQUEST.SelectedValue	= "";
			DDL_LEGAL_OFFICER.SelectedValue	= "";
		}
				...
				try
				{
					...
					if (dt1.Rows.Count > 0)
					{
						for (int i = 0; i < dt1.Rows.Count; i++)
						{
							...
							/* Procedure :
							 * LGAM_ADMIN_DOCUMENTATION
							 * LGAM_PENDING_TASK_LIST
							 * LGAM_SLA_REPORTING
							 * */
							...
							conn.QueryString = "EXEC " + proc + " " + tools.ConvertDate(TXT_TGL1.Text, DDL_BLN1.SelectedValue, TXT_THN1.Text) + "," +
												tools.ConvertDate(TXT_TGL2.Text, DDL_BLN2.SelectedValue, TXT_THN2.Text) + ",'" + 
												DDL_UNIT_REQUEST.SelectedValue + "','" + DDL_LEGAL_OFFICER.SelectedValue + "'";
							conn.ExecuteQuery();
							...
							if (dt3.Rows.Count > 0)
							{
								...
								conn.QueryString = "SELECT CELL_ROW, CELL_COL, DB_FIELD FROM DOCEXPORT_TEMPLATE_DETAIL WHERE TEMPLATE_ID = '" + templateid + "' AND SHEET_ID = '" + sheetid + "' AND SHEET_SEQ = '" + sheetseq + "' ORDER BY CELL_COL";
								conn.ExecuteQuery();
		...
		private void DATA_EXPORT_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch (((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					try 
					{					
						conn.QueryString = "SELECT TOP 1 UPLOAD_PATH FROM VW_DOCEXPORT_PARAMETER WHERE TEMPLATE_GROUP = 'LGAMREPORTING'";
						conn.ExecuteQuery();
