	...
	public partial class Main : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				{
					TXT_NOTA_KIRIM.Enabled			= false;
					TXT_TGL_KIRIM_DAY.Enabled		= false;
					DDL_TGL_KIRIM_MONTH.Enabled		= false;
					TXT_TGL_KIRIM_YEAR.Enabled		= false;
					TXT_REMARK.Enabled				= false;
					BTN_SAVE.Enabled				= false;
					BTN_UPDATE_FINISH.Enabled		= false;
				}
			}
			FillDGRDoc();
			ViewUploadFiles();
		...
		{
			conn.QueryString = "SELECT * FROM VW_LGAM_RESULT_LIST WHERE CU_REF = '" + Request.QueryString["curef"] + "'";
			conn.ExecuteQuery();
			...
			if(conn.GetFieldValue("AP_CURRTRACK").ToString() == "LGL.5")
			{
				BTN_SAVE.Enabled				= false;
				BTN_UPDATE_FINISH.Enabled		= false;
			}
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			if (TXT_TGL_KIRIM_DAY.Text != "" && DDL_TGL_KIRIM_MONTH.SelectedValue != "" && TXT_TGL_KIRIM_YEAR.Text != "") 
			{
				if (!GlobalTools.isDateValid(TXT_TGL_KIRIM_DAY.Text, DDL_TGL_KIRIM_MONTH.SelectedValue, TXT_TGL_KIRIM_YEAR.Text)) 
				{
					GlobalTools.popMessage(this, "Tanggal Kirim Tidak Valid!");
					return;
				}
			}
			...
			try
			{
				TGL = tools.ConvertDate(TXT_TGL_KIRIM_DAY.Text, DDL_TGL_KIRIM_MONTH.SelectedValue, TXT_TGL_KIRIM_YEAR.Text);
				
				conn.QueryString = "UPDATE LGAM_LEGAL_ADMIN SET NOTA_RESULT = '" + TXT_NOTA_KIRIM.Text + "', SEND_RESULT = " + TGL + ", REMARK_RESULT = '" + TXT_REMARK.Text + "' WHERE CU_REF = '" + Request.QueryString["curef"] + "'";
				conn.ExecuteQuery();

				BTN_UPDATE_FINISH.Enabled		= true;
			}
			catch(Exception ex)
			{
		...
		protected void BTN_UPDATE_FINISH_Click(object sender, System.EventArgs e)
		{
			try
			{
