...
namespace SME.Legal.LegalAdviseAdministration.InquiryHistoricalData
{
	...
	public partial class ResultList : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "SELECT * FROM VW_LGAM_HISTORICAL_LIST";
		...
		private void BindData(string dataGridName, string strconn)
		{
			if(strconn != "")
			{
				...
				for (int i = 0; i < DATA_GRID.Items.Count; i++)
				{
					...
					DATA_GRID.Items[i].Cells[3].Text = tools.FormatDate(DATA_GRID.Items[i].Cells[3].Text, true);
				}
		...
		private void SearchData()
		{
			...
			if(query != "")
			{
				conn.QueryString = "SELECT DISTINCT * FROM VW_LGAM_HISTORICAL_LIST WHERE 1=1 " + query + "ORDER BY SEQ";
				BindData(DATA_GRID.ID.ToString(), conn.QueryString);
			}

			else
			{
				conn.QueryString = "SELECT DISTINCT * FROM VW_LGAM_HISTORICAL_LIST ORDER BY SEQ";
				BindData(DATA_GRID.ID.ToString(), conn.QueryString);
			}
		}
	}
