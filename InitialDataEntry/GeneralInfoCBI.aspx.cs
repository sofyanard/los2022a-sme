...
namespace SME.InitialDataEntry
{
	...
	/// </summary>
	public partial class GeneralInfoCBI : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SELECT * FROM CUSTOMER WHERE CU_REF = '" + Request.QueryString["curef"] + "'";
			conn.ExecuteQuery();
			...
			if (!IsPostBack)
			{
				...
				if ((conn.GetFieldValue("sg_BUSSUNITid") == "CB100") ||
					(conn.GetFieldValue("sg_BUSSUNITid") == "MD100") ||
					(conn.GetFieldValue("sg_BUSSUNITid") == "SM100") ||
					(conn.GetFieldValue("sg_BUSSUNITid") == "CR100") ||
                    (conn.GetFieldValue("sg_BUSSUNITid") == "MB100") ||
                    (conn.GetFieldValue("sg_BUSSUNITid") == "BPSML") ||
					(conn.GetFieldValue("sg_BUSSUNITid") == "BPMCR"))
				...
				//	if (temp_grpunit.Text != "CO" && Request.QueryString["mc"] != "030")
				...
				try { DDL_AP_GRSALESCURR.SelectedValue = "IDR"; } 
				catch { }
			...
			TR_ISIGENINFO.Visible = false;
            try { DDL_PROG_CODE.SelectedIndex = 1; }
            catch { }
            try { DDL_AP_SRCCODE.SelectedIndex = 1; }
            catch { }
		}
		...
		{
			conn.QueryString = "SELECT WASPADA FROM CUSTOMER WHERE CU_REF ='" + Request.QueryString["curef"] + "'";
			conn.ExecuteQuery();
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			if (RDO_RFCUSTOMERTYPE.SelectedValue == "01")	// badan usaha
			{
				...
				try 
				{
					conn.QueryString = "exec IDE_GENINFO_COMP_INSERT3 '" + 
					...
					conn.QueryString = "exec IDE_GENINFO_COMP_INSERT4 '" + 
						curef + "', " + 
			...
			else	
			{
				...
				try 
				{
					/*conn.QueryString = "exec IDE_GENINFO_PERSON_INSERT4 '" + 
						TXT_CU_REF.Text + "', '" + 
						LBL_AP_RELMNGR.Text + "', '" + 
						RDO_RFCUSTOMERTYPE.SelectedValue + "', '" +
						TXT_CU_CIF_P.Text + "', '" +
						...
						tool.ConvertDate(TXT_CU_IDCARDEXP_DAY.Text, DDL_CU_IDCARDEXP_MONTH.SelectedValue, TXT_CU_IDCARDEXP_YEAR.Text) + ", " + 
						tool.ConvertNull(DDL_CU_JOBTITLE.SelectedValue) + ", '" + 
						...
						TXT_CU_MOTHER.Text+"',"+
						//pipeline
						tool.ConvertFloat(Textbox_skbngpasar.Text) + ", " + 
						tool.ConvertFloat(Textbox_skbngminta.Text) + ", '" + 
						DDL_groupnasabah.Text + "', '" +
						DDL_bmsektor.SelectedValue + "', '" +
						DDL_bmsubsektor.SelectedValue + "', '" +
						DDL_bmsubsubsektor.SelectedValue + "', " +
					...
					conn.QueryString = "exec IDE_GENINFO_PERSON_INSERT7 '" + 
						Request.QueryString["curef"] + "', '" + 
						//LBL_AP_RELMNGR.Text + "', '" + 
						RDO_RFCUSTOMERTYPE.SelectedValue + "', '" +
						TXT_CU_CIF_P.Text + "', '" +
					...
					conn.QueryString = "exec IDE_GENINFO_PERSON_INSERT5 '" + TXT_CU_REF.Text + "', " + 
					...
					conn.QueryString = "exec IDE_GENINFO_PERSON_INSERT6 '" + TXT_CU_REF.Text + "', " + 
			...
			/*
            if (Request.QueryString["sta"] != "prev" && Request.QueryString["mc"] != "030")
			{
				if (temp_grpunit.Text !="CO")
		...
		private void DDL_AP_BUSINESSUNIT_SelectedIndexChanged(object sender, System.EventArgs e)
		{
		...
		private void DDL_AP_SALESAGENCY_SelectedIndexChanged(object sender, System.EventArgs e)
		{
		...
		private void RDO_BI_CHECKING_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			if (RDO_BI_CHECKING.SelectedValue == "0") // No Request BI Checking
			{
				LBL_CO.Visible = false;
		...
		private void TXT_CON_TextChanged(object sender, System.EventArgs e)
		{
			Response.Redirect("../InitialDataEntry/FindCustomer.aspx?mc="+Request.QueryString["mc"]+"&tc=" + Request.QueryString["tc"]);
		}

		private void DDL_PROG_CODE_SelectedIndexChanged(object sender, System.EventArgs e)
		{				
			//////////////////////////////////////////////
			/// TODO : Please don't hard code !!!!
			/// 			
		...
		private void CHB_CU_NAMAPELAPORAN_CheckedChanged(object sender, System.EventArgs e)
		{
		...
		private void DDL_CU_COMPEXTRATING_BY_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			GlobalTools.fillRefList(DDL_CU_COMPEXTRATING_CLASS, "SELECT RTGCLASS_CODE, RTGCLASS_DESC FROM RFEXTERNALRATINGCLASS WHERE RTGCOMP_CODE = '" + DDL_CU_COMPEXTRATING_BY.SelectedValue + "' ORDER BY RTGCLASS_CODE", false, conn);
		}
