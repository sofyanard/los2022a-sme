...
//				conn.ExecuteQuery();
		...
		{
			conn.QueryString = "select * from VW_BACKUP_COLL_RE "+
			...
			else
			{
				try 
				{
					conn.QueryString = "exec DE_COLL_RE '"+ LBL_CUREF.Text +"', "+ LBL_CL_SEQ.Text +", 0, "+
						...
						TXT_CL_OWNER.Text +"', "+ tool.ConvertNull(DDL_CL_RELATIONSHIP.SelectedValue) +", "+ tool.ConvertFloat(TXT_CL_RESULTVAL.Text) +", "+
						tool.ConvertFloat(TXT_CL_MARKETVAL.Text) +", "+ tool.ConvertFloat(TXT_CL_PPAPVAL.Text) +", "+ tool.ConvertNull(DDL_CL_DEVELOPER.SelectedValue) +", '"+
