...
namespace SME.HistoricalLoanInfo.Collateral
{
	...
	public partial class Jaminan_List : System.Web.UI.Page
	{
		...
		private void ViewData()
		{
			...
			conn.QueryString = "select lc.cl_seq, cl.cl_type, ct.coltypedesc, ct.collinktable, cl.cl_desc, lc.ap_regno " +
                "from listcollateral lc inner join collateral cl on lc.cu_ref = cl.cu_ref and lc.cl_seq = cl.cl_seq join " +
                "rfcollateraltype ct on cl.cl_type = ct.coltypeseq where ap_regno='" + Request.QueryString["regno"] + "'";
			*/
			//"case isnull(lc.AP_REGNO,'') when '' then ca.AP_REGNO else lc.AP_REGNO end as AP_REGNO "+
			
			conn.QueryString = "select * from vw_BACKUP_JAMINAN_LIST "+
				"where lc_AP_REGNO = '" + Request.QueryString["regno"] + "' or ca_AP_REGNO = '" + Request.QueryString["regno"] + "' ";
			conn.ExecuteQuery();
			...
			for (int i = 0; i < row; i++)
			{
				...
				*/
				...
				t.NavigateUrl = conn.GetFieldValue(i,"collinktable") +".aspx?regno="+regno+"&curef="+ curef +"&coltypeid="+ cl_type +"&CL_SEQ="+ cl_seq + "&de=" + Request.QueryString["de"];
				//------------------------------
				t.Target = "coldetail";
				...
				if (((CheckBox)Table_List.FindControl("check"+ i)).Checked == true)
				{
					...
					else
					{
						conn.QueryString = "exec DE_COLL '2', '"+ Request.QueryString["curef"] + "', "+CL_SEQdel+
							", "+((TextBox)Table_List.FindControl("coltype"+ i)).Text +", 0, '', null, null, null" ;
						conn.ExecuteQuery();
					}
				}
			}
