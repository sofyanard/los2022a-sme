...
namespace SME.Booking.Channeling
{
	...
	public partial class AssignmentComplyMain : System.Web.UI.Page
	{
		...
		private void ViewMenu()
		{
			try 
			{
				conn.QueryString = "select SM_ID,MENUCODE,SM_MENUDISPLAY,SM_LINKNAME from SCREENMENU where menucode = 'CHAN007'";
				conn.ExecuteQuery();
				for (int i = 0; i < conn.GetRowCount(); i++) 
				{
		...
		protected void BTN_RETURNTOBU_Click(object sender, System.EventArgs e)
		{
			...
			conn.ExecuteQuery();
			Response.Redirect("ListBooking.aspx?msg=notok&regno=" + Request.QueryString["regno"]);
		}
